...
def get(doctype, id=None, filters=None, parent=None):
    """Returns a document by id or filters
...
def get_value(doctype, fieldname, filters=None, as_dict=True, debug=False, parent=None):
    ...
    if not frappe.has_permission(doctype, parent_doctype=parent):
        frappe.throw(
            _("No permission for {0}").format(_(doctype)), frappe.PermissionError
        )
...
@frappe.whitelist(methods=["POST", "PUT"])
def reid_doc(doctype, old_id, new_id, merge=False):
    """Reid document

    :param doctype: DocType of the document to be reided
    :param old_id: Current `id` of the document to be reided
    :param new_id: New `id` to be set"""
    new_id = frappe.reid_doc(doctype, old_id, new_id, merge=merge)
    return new_id
...
@frappe.whitelist()
def has_permission(doctype, docid, perm_type="read"):
...
@frappe.whitelist()
def get_doc_permissions(doctype, docid):
...
def validate_link(doctype: str, docid: str, fields=None):
        ...
        try:
            ...
            frappe.msgprint(
                _("Document {0} {1} does not exist").format(
                    frappe.bold(doctype), frappe.bold(docid)
                ),
            )
        return values
    ...
    if not values.id or not fields:
        return values
