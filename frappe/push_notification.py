...
class PushNotification:
    ...
    def add_topic(self, topic_id: str) -> bool:
        """
        ...
        """
        data = self._send_post_request(
            "notification_relay.api.topic.add", {"topic_id": topic_id}
        )
        return data["success"]
    ...
    def remove_topic(self, topic_id: str) -> bool:
        ...
        """
        data = self._send_post_request(
            "notification_relay.api.topic.remove", {"topic_id": topic_id}
        )
        return data["success"]

    def subscribe_topic(self, user_id: str, topic_id: str) -> bool:
        """
        Subscribe a user to a topic.

        :param user_id: (str) The ID of the user. This should be user's unique identifier.
        :param topic_id: (str) The id of the topic. This topic should be already created.
        :return: bool True if successful, False otherwise.
        """
        data = self._send_post_request(
    ...
    def unsubscribe_topic(self, user_id: str, topic_id: str) -> bool:
        """
    ...
    def _get_credential(self) -> tuple[str, str]:
        ...
        if notification_settings.api_key and notification_settings.api_secret:
            return notification_settings.api_key, notification_settings.get_password(
                "api_secret"
            )
        ...
        frappe.cache().set_value(
            f"{self._site_name}:push_relay_registration_token",
            token,
            expires_in_sec=600,
        )
        body = {
        ...
        response = self._send_post_request(
            "notification_relay.api.auth.get_credential", body, False
        )
    ...
    def _send_post_request(
        self, method: str, params: dict, use_authentication: bool = True
    ):
        """
        ...
        if use_authentication:
            ...
            client = FrappeClient(
                relay_server_endpoint, api_key=api_key, api_secret=api_secret
            )
        else:
...
def subscribe(fcm_token: str, project_id: str) -> dict:
    success, message = PushNotification(project_id).add_token(
        frappe.session.user, fcm_token
    )
    return {"success": success, "message": message}


@frappe.whitelist(methods=["GET"])
def unsubscribe(fcm_token: str, project_id: str) -> dict:
    success, message = PushNotification(project_id).remove_token(
        frappe.session.user, fcm_token
    )
    return {"success": success, "message": message}
