...
class PushNotification:
    ...
    def add_topic(self, topic_name: str) -> bool:
        ...
        """
        data = self._send_post_request(
            "notification_relay.api.topic.add", {"topic_name": topic_name}
        )
        return data["success"]
    ...
    def remove_topic(self, topic_name: str) -> bool:
        ...
        """
        data = self._send_post_request(
            "notification_relay.api.topic.remove", {"topic_name": topic_name}
        )
        return data["success"]
...
def unsubscribe(fcm_token: str, project_name: str) -> dict:
    success, message = PushNotification(project_name).remove_token(
        frappe.session.user, fcm_token
    )
    return {"success": success, "message": message}
