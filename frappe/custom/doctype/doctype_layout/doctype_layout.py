...
class DocTypeLayout(Document):
    ...
    def add_fields(
        self, added_fields: list[str], doctype_fields: list["DocField"]
    ) -> list[dict]:
        added = []
        for field in added_fields:
            field_details = next(
                (f for f in doctype_fields if f.fieldname == field), None
            )
            ...
            if field_details.get("insert_after"):
                insert_after = next(
                    (
                        ...
                        if f.fieldname == field_details.insert_after
                    ),
                    None,
                )
