...
def get_columns():
    ...
    columns = [
        ...
        {
            "label": "Hook Values (resolved)",
            "fieldname": "hook_values",
            "fieldtype": values_field_type,
        },
    ]
    ...
    columns += [
        {"label": app, "fieldname": app, "fieldtype": values_field_type}
    ...
    for hook, values in hooks.items():
        if isinstance(values, dict):
            for k, v in values.items():
                row = {
                    "hook_name": hook,
                    "hook_key": fmt_hook_values(k),
                    "hook_values": fmt_hook_values(v),
                }
                for app in installed_apps:
