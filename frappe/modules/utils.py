...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
    ...
    if not frappe.flags.in_import and is_standard and frappe.conf.developer_mode:
        ...
        # json
        export_to_files(
            record_list=[[doc.doctype, doc.id]],
...
def export_customizations(
    ...
    custom = {
        "custom_fields": frappe.get_all(
            "Custom Field", fields="*", filters={"dt": doctype}, order_by="id"
        ),
        ...
        "links": frappe.get_all(
            "DocType Link", fields="*", filters={"parent": doctype}, order_by="id"
        ),
        "doctype": doctype,
...
def scrub_dt_dn(dt: str, dn: str) -> tuple[str, str]:
    """Returns in lowercase and code friendly ids of doctype and id for certain types"""
...
def get_doctype_module(doctype: str) -> str:
    """Returns **Module Def** id of given doctype."""
...
def make_boilerplate(
    ...
    template_id = template.replace("controller", scrub(doc.id))
    if template_id.endswith("._py"):
        template_id = template_id[:-4] + ".py"
    target_file_path = os.path.join(target_path, template_id)
