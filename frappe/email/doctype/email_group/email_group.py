...
class EmailGroup(Document):
    ...
    def onload(self):
        ...
        self.get("__onload").import_types = [
            ...
            for d in frappe.get_all(
                "DocField", ("parent", "label"), {"options": "Email"}
            )
    ...
    def import_from(self, doctype):
        ...
        email_field = next(
            ...
            if d.fieldtype in ("Data", "Small Text", "Text", "Code")
            and d.options == "Email"
        )
        ...
        for user in frappe.get_all(
            doctype, [email_field, unsubscribed_field or "name"]
        ):
            with contextlib.suppress(
                frappe.UniqueValidationError, frappe.InvalidEmailAddressError
            ):
                email = (
                    parse_addr(user.get(email_field))[1]
                    ...
                    frappe.get_doc(
                        {
                            ...
                            "email": email,
                            "unsubscribed": (
                                user.get(unsubscribed_field)
                                if unsubscribed_field
                                else 0
                            ),
                        }
    ...
    def get_welcome_url(self, email: str | None = None) -> str | None:
        ...
        return (
            add_query_params(
                self.welcome_url, {"email": email, "email_group": self.name}
            )
    ...
    def on_trash(self):
        for d in frappe.get_all(
            "Email Group Member", "name", {"email_group": self.name}
        ):
