...
class EmailQueue(Document):
    ...
    def clear_old_logs(days=30):
        ...
        # Delete queue table
        (frappe.qb.from_(email_queue).delete().where(email_queue.creation < (Now() - Interval(days=days)))).run()
        ...
        (
            frappe.qb.from_(email_recipient).delete().where(email_recipient.creation < (Now() - Interval(days=days)))
        ).run()
...
class SendMailContext:
    ...
    def get_tracker_str(self, recipient_email) -> str:
        ...
        elif self.email_account_doc and self.email_account_doc.track_email_status and self.queue_doc.communication:
