...
class TestNotification(IntegrationTestCase):
	...
	def test_new_and_save(self):
        ...
        self.assertEqual(
            frappe.db.get_value("Communication", communication.id, "subject"),
            "__testing__",
        )
    ...
    def test_date_changed(self):
        ...
        frappe.get_doc(
            "Scheduled Job Type",
            dict(
                method="frappe.email.doctype.notification.notification.trigger_daily_alerts"
            ),
        ).execute()
    ...
    def test_notification_to_assignee(self):
        ...
        email_queue = frappe.get_doc(
            "Email Queue", {"reference_doctype": "ToDo", "reference_id": todo.id}
        )
