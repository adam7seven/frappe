...
frappe.ui.form.on("Auto Email Report", {
    ...
    fetch_report_filters(frm) {
        if (frm.doc.report && frm.doc.report_type !== "Report Builder" && frm.script_setup_for !== frm.doc.report) {
            frappe.call({
    ...
    show_filters: async function (frm) {
        ...
        if (
            ...
            (frm.doc.report_type !== "Report Builder" && reference_report && reference_report.filters)
        ) {
            ...
            if (frm.doc.report_type === "Custom Report" && reference_report && reference_report.filters) {
                if (frm.doc.filters) {
                ...
                if (val.fieldtype != "Break") {
                    if (val.fieldtype === "MultiSelectList") {
                        val.get_data = txt => {
                            ...
                            const doctype_link =
                                frappe.scrub(val.options) === val.options ? dialog.get_value(val.options) : val.options;

                            return doctype_link ? frappe.db.get_link_options(doctype_link, txt) : [];
                        };
                    }
