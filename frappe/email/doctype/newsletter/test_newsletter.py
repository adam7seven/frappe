...
class TestNewsletterMixin:
    ...
    def setup_email_group(self):
        if not frappe.db.exists("Email Group", "_Test Email Group"):
            frappe.get_doc(
                {"doctype": "Email Group", "title": "_Test Email Group"}
            ).insert()
    ...
    def get_newsletter(**kwargs) -> "Newsletter":
        ...
        newsletter = frappe.get_doc(
            {"doctype": doctype, **newsletter_content, **kwargs}
        )
...
class TestNewsletter(TestNewsletterMixin, IntegrationTestCase):
    ...
    def test_unsubscribe(self):
        ...
        group = frappe.get_all(
            "Newsletter Email Group", filters={"parent": id}, fields=["email_group"]
        )
    ...
    def test_newsletter_with_no_recipient(self):
        ...
        with patch(
            property_path, new_callable=PropertyMock
        ) as mock_newsletter_recipients:
