...
class Newsletter(WebsiteGenerator):
    ...
    def get_sending_status(self):
        ...
        emails_queued = is_job_queued(
            job_id=frappe.utils.get_job_id(
                "send_bulk_emails_for", self.doctype, self.id
            ),
            queue="long",
        )
        return {
    ...
    def validate_newsletter_status(self):
        if self.email_sent:
            frappe.throw(
                _("Newsletter has already been sent"), exc=NewsletterAlreadySentError
            )

        if self.get("__islocal"):
            frappe.throw(
                _("Please save the Newsletter before sending"),
                exc=NewsletterNotSavedError,
            )

    def validate_newsletter_recipients(self):
        if not self.newsletter_recipients:
            frappe.throw(
                _("Newsletter should have atleast one recipient"),
                exc=NoRecipientFoundError,
            )
    ...
    def validate_publishing(self):
        if self.send_webview_link and not self.published:
            frappe.throw(
                _("Newsletter must be published to send webview link in email")
            )
    ...
    def validate_scheduling_date(self):
        ...
        if (
            ...
            and frappe.utils.get_datetime(self.schedule_send)
...
@frappe.whitelist(allow_guest=True)
def newsletter_email_read(
    recipient_email=None, reference_doctype=None, reference_id=None
):
