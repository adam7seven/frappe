    ...
    def decorator_cache_email_account(func):
        ...
        def wrapper_cache_email_account(*args, **kwargs):
            ...
            matched_accounts = list(
                filter(None, [cached_accounts.get(key) for key in match_by])
            )
            if matched_accounts:
...
class EmailAccount(Document):
    ...
    def validate(self):
        ...
        if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
            frappe.throw(
                _("You need to set one IMAP folder for {0}").format(
        ...
        validate_oauth = use_oauth and not (
            self.is_new() and not self.get_oauth_token()
        )
    ...
    def get_domain_values(self, domain: str):
        return frappe.db.get_value(
            "Email Domain", domain, EMAIL_DOMAIN_FIELDS, as_dict=True
        )
    ...
    def find_outgoing(
        cls, match_by_email=None, match_by_doctype=None, _raise_error=False
    ):
    ...
    def _frappe_mail_client(self):
        if self.auth_method == "OAuth":
            if access_token := self.get_access_token():
                return FrappeMail(
                    self.frappe_mail_site, self.email_id, access_token=access_token
                )
            ...
            return FrappeMail(
                self.frappe_mail_site,
                self.email_id,
                self.api_key,
                self.get_password("api_secret"),
            )
    ...
    def handle_incoming_connect_error(self, description):
        if self.get_failed_attempts_count() > 5:
            ...
            frappe.enqueue(
                self._disable_broken_incoming_account, description=description
            )
        else:
    ...
    def set_failed_attempts_count(self, value):
        frappe.cache.set(f"{self.id}:email-account-failed-attempts", value)

    def get_failed_attempts_count(self):
        return cint(frappe.cache.get(f"{self.id}:email-account-failed-attempts"))
                ...
                try:
                    ...
                    if self.use_imap:
                        self.handle_bad_emails(
                            mail.uid, mail.raw_message, frappe.get_traceback()
                        )
    ...
    def get_inbound_mails(self) -> list[InboundMail]:
        ...
        try:
            ...
            self.log_error(
                title=_("Error while connecting to email account {0}").format(self.id)
            )
            return []
...
def get_append_to(
    ...
    email_append_to_list = [
        ...
        for dt in frappe.get_all(
            "DocType", filters=filters, fields=["id", "email_append_to"]
        )
    ]
...
def notify_unreplied():
    ...
    for email_account in frappe.get_all(
        ...
        if email_account.use_imap:
            append_to = [
                folder.get("append_to") for folder in email_account.imap_folder
            ]
        else:
        ...
        if append_to:
            ...
            for comm in frappe.get_all(
                ...
                if (
                    frappe.db.get_value(
                        comm.reference_doctype, comm.reference_id, "status"
                    )
                    == "Open"
                ):
...
def pull(now=False):
    ...
    for email_account in email_accounts:
        ...
        if now:
            ...
            # job_id is used to prevent duplicates in queue
            job_id = f"pull_from_email_account|{email_account.id}"

            queued_jobs = get_jobs(site=frappe.local.site, key="job_id")[
                frappe.local.site
            ]
...
def setup_user_email_inbox(
    ...
    user_ids = frappe.db.get_values("User", {"email": email_id}, as_dict=True)
    if not user_ids:
        return
    ...
    for user in user_ids:
        ...
        user_inbox = (
            frappe.db.get_value(
                "User Email",
                {"email_account": email_account, "parent": user_id},
                ["id"],
            )
            or None
        )
    ...
    if update_user_email_settings:
        ...
        ).set(UserEmail.enable_outgoing, (enable_outgoing or 0)).set(
        ...
        users = " and ".join([frappe.bold(user.get("id")) for user in user_ids])
