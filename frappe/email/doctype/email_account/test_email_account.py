...
class TestEmailAccount(IntegrationTestCase):
    ...
    def mocked_get_inbound_mails(email_account, messages=None, mocked_logout=None, mocked_select_imap_folder=None):
    ...
    @patch("frappe.email.receive.EmailServer.select_imap_folder", return_value=True)
    @patch("frappe.email.receive.EmailServer.logout", side_effect=lambda: None)
    def mocked_email_receive(email_account, messages=None, mocked_logout=None, mocked_select_imap_folder=None):
        if messages is None:
            messages = {}
