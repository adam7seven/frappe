...
class TestEmailAccount(FrappeTestCase):
    ...
    def test_incoming(self):
        ...
        self.assertTrue(
            frappe.db.get_value(comm.reference_doctype, comm.reference_id, "id")
        )
    ...
    def test_threading(self):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "test_mails", "reply-1.raw")
        ) as f:
            raw = f.read()
    ...
    def test_threading_by_subject(self):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "test_mails", "reply-2.raw")
        ) as f:
            test_mails = [f.read()]

        with open(
            os.path.join(os.path.dirname(__file__), "test_mails", "reply-3.raw")
        ) as f:
    ...
    def test_threading_by_message_id(self):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "test_mails", "reply-4.raw")
        ) as f:
            messages = {
                # append_to = ToDo
                '"INBOX"': {
                    "latest_messages": [
                        f.read().replace(
                            "{{ message_id }}", "<" + last_mail.message_id + ">"
                        )
                    ],
                    "seen_status": {2: "UNSEEN"},
                    "uid_list": [2],
    ...
    def test_append_to(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                communication.reference_doctype, communication.reference_id
            )
        )
...
class TestInboundMail(FrappeTestCase):
    ...
    def test_reference_document(self):
        ...
        queue_record = self.new_email_queue(
            reference_doctype="ToDo", reference_id=todo.id
        )
        mail_content = self.get_test_mail(fname="reply-4.raw").replace(
            "{{ message_id }}", queue_record.message_id
        )
    ...
    def test_reference_document_by_record_id_in_subject(self):
        ...
        mail_content = self.get_test_mail(
            fname="incoming-subject-placeholder.raw"
        ).replace("{{ subject }}", f"RE: (#{todo.id})")
