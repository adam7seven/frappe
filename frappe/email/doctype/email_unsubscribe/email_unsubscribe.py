...
class EmailUnsubscribe(Document):
    ...
    def validate(self):
        if not self.global_unsubscribe and not (
            self.reference_doctype and self.reference_id
        ):
            frappe.throw(
                _("Reference DocType and Reference ID are required"),
                frappe.MandatoryError,
            )

        if not self.global_unsubscribe and frappe.db.get_value(
            self.doctype, self.id, "global_unsubscribe"
        ):
        ...
        if self.global_unsubscribe:
            if frappe.get_all(
                ...
                frappe.throw(
                    _("{0} already unsubscribed").format(self.email),
    ...
    def on_update(self):
        if self.reference_doctype and self.reference_id:
            ...
            doc.add_comment(
                "Label", _("Left this conversation"), comment_email=self.email
            )
