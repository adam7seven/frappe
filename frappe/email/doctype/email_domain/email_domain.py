...
def get_error_message(event):
    return {
        "incoming": (
            _("Incoming email account not correct"),
            _("Error connecting via IMAP/POP3: {e}"),
        ),
        "outgoing": (
            _("Outgoing email account not correct"),
            _("Error connecting via SMTP: {e}"),
        ),
    }[event]
...
class EmailDomain(Document):
    ...
    def on_update(self):
        ...
        for email_account in frappe.get_all(
            "Email Account", filters={"domain": self.id}
        ):
            try:
                ...
                frappe.msgprint(
                    _("Error has occurred in {0}").format(email_account.id),
