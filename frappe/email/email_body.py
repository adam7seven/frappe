...
class EMail:
    ...
    def validate(self):
        ...
        self.reply_to = validate_email_address(
        ...
        self.recipients = [
            strip(r) for r in self.recipients if r not in frappe.STANDARD_USERS
        ]
    ...
    def replace_sender(self):
        if cint(self.email_account.always_use_account_email_id_as_sender):
            ...
            self.sender = email.utils.formataddr(
                (
                    str(Header(sender_name or self.email_account.id, "utf-8")),
    ...
    def replace_sender_name(self):
        if cint(self.email_account.always_use_account_name_as_sender_name):
            ...
            self.sender = email.utils.formataddr(
                (str(Header(self.email_account.id, "utf-8")), sender_email)
            )
    ...
    def make(self):
        ...
        headers = {
            ...
            "CC": (
                ", ".join(self.cc)
                if self.cc and self.expose_recipients == "header"
                else None
            ),
...
def get_formatted_html(
    ...
    rendered_email = frappe.get_template("templates/emails/standard.html").render(
        {
            "brand_logo": (
                get_brand_logo(email_account) if with_container or header else None
            ),
...
def get_message_id():
    """Returns Message ID created from doctype and id"""
