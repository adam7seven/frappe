...
class EmailServer:
    ...
    def connect_pop(self):
        ...
        try:
            if cint(self.settings.use_ssl):
                ...
                self.pop = poplib.POP3(self.settings.host, self.settings.incoming_port, timeout=frappe.conf.pop_timeout)
    ...
    def get_messages(self, folder="INBOX"):
        ...
        if self.settings.use_imap:
            out.update({"uid_list": email_list, "seen_status": self.seen_status, "uid_reindexed": self.uid_reindexed})
    ...
    def is_temporary_system_problem(self, e):
        ...
        return any(
            message in strip(cstr(e)) or message in strip(cstr(getattr(e, "strerror", ""))) for message in messages
        )
...
class Email:
    ...
    def get_payload(self, part):
        ...
        try:
            ...
            try:
                return str(part.get_payload(decode=True), ALTERNATE_CHARSET_MAP.get(charset, "utf-8"), "ignore")
            except Exception:
