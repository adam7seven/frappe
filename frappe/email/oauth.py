...
class Oauth:
    ...
    def _validate(self) -> None:
        if not self._access_token:
            frappe.throw(
                frappe._("Please Authorize OAuth for Email Account {}").format(
                    self.email_account
                ),
    ...
    def _connect_pop(self) -> None:
        ...
        res = self._conn._shortcmd(
            "AUTH {} {}".format(
                self._mechanism,
                base64.b64encode(bytes(self._auth_string, "utf-8")).decode("utf-8"),
            )
        )
    ...
    def _connect_smtp(self) -> None:
        self._conn.auth(
            self._mechanism, lambda x: self._auth_string, initial_response_ok=False
        )
