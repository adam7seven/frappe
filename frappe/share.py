...
@frappe.whitelist()
def add(
    doctype, id, user=None, read=1, write=0, submit=0, share=0, everyone=0, notify=0
):
...
def remove(doctype, id, user, flags=None):
    share_id = frappe.db.get_value(
        "DocShare", {"user": user, "share_id": id, "share_doctype": doctype}
    )
...
def set_permission(doctype, id, user, permission_to, value=1, everyone=0):
    ...
    return set_docshare_permission(
        doctype, id, user, permission_to, value=value, everyone=everyone
    )


def set_docshare_permission(
    doctype, id, user, permission_to, value=1, everyone=0, flags=None
):
    ...
    if not share_id:
        if value:
            share = add_docshare(
                doctype, id, user, everyone=everyone, **{permission_to: 1}, flags=flags
            )
        else:
...
def get_share_id(doctype, id, user, everyone):
    if cint(everyone):
        share_id = frappe.db.get_value(
            "DocShare", {"everyone": 1, "share_id": id, "share_doctype": doctype}
        )
    else:
        share_id = frappe.db.get_value(
            "DocShare", {"user": user, "share_id": id, "share_doctype": doctype}
...
def check_share_permission(doctype, id):
    ...
    if not frappe.has_permission(doctype, ptype="share", doc=id):
        frappe.throw(
            _("No permission to {0} {1} {2}").format("share", _(doctype), id),
