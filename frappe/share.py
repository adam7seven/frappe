...
@frappe.whitelist()
def add(
    doctype, name, user=None, read=1, write=0, submit=0, share=0, everyone=0, notify=0
):
...
def set_permission(doctype, name, user, permission_to, value=1, everyone=0):
    ...
    return set_docshare_permission(
        doctype, name, user, permission_to, value=value, everyone=everyone
    )
...
def check_share_permission(doctype, name):
    ...
    if not frappe.has_permission(doctype, ptype="share", doc=name):
        frappe.throw(
            _("No permission to {0} {1} {2}").format("share", _(doctype), name),
