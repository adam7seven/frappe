...
def has_permission(
    ...
    *,
    ...
    if doc:
        ...
        debug and _debug_log(
            "User has following permissions using role permission system: " + frappe.as_json(role_permissions, indent=8)
        )
...
def get_doc_permissions(doc, user=None, ptype=None, debug=False):
    ...
    debug and _debug_log(
        "User has following permissions using role permission system: " + frappe.as_json(permissions, indent=8)
    )
    ...
    if permissions.get("has_if_owner_enabled"):
        ...
        debug and _debug_log("User is owner of document, so permissions are updated to: " + frappe.as_json(permissions))
    ...
    debug and _debug_log(
        "Final applicable permissions after evaluating user permissions: " + frappe.as_json(permissions, indent=8)
    )
    return permissions
    ...
    def get():
        if user == "Administrator":
            ...
            roles = (
                ...
                .where((table.parenttype == "User") & (table.parent == user) & (table.role.notin(AUTOMATIC_ROLES)))
                .select(table.role)
                .run(pluck=True)
            )
...
def update_permission_property(
    ...
    custom_docperm = frappe.db.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel))
    if custom_docperm:
...
def add_permission(doctype, role, permlevel=0, ptype=None):
    ...
    if frappe.db.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel, if_owner=0)):
        frappe.msgprint(
...
def has_child_permission(
    ...
    *,
    ...
    if parent_meta.istable or not (
        valid_parentfields := [df.fieldname for df in parent_meta.get_table_fields() if df.options == child_doctype]
    ):
        push_perm_check_log(
