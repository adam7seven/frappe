...
def remove_user_permission(doctype, id, user):
    ...
    frappe.delete_doc("User Permission", user_permission_id)
...
def clear_user_permissions_for_doctype(doctype, user=None):
    ...
    for d in user_permissions_for_doctype:
        frappe.delete_doc("User Permission", d.id)
...
def update_permission_property(
    ...
    frappe.qb.update(table).set(ptype, value).where(table.id == id).run()
