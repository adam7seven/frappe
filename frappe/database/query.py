...
class Engine:
                    ...
                    if _operator in initial_fields:
                        ...
                        if isinstance(operator_mapping, BuiltinFunctionType):
                            ...
                            field = operator_mapping(
                                *map(
                                    lambda field: (
                                        Field(field.strip()) if "`" not in field else PseudoColumnMapper(field.strip())
                                    ),
                                    arg.split(_operator),
                                ),
                            )
    ...
    def sanitize_fields(self, fields: str | list | tuple):
        if isinstance(fields, list | tuple):
            return [_sanitize_field(field, self.is_mariadb) if isinstance(field, str) else field for field in fields]
