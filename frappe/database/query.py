    ...
    def apply_order_by(self, order_by: str | None):
        ...
        for declaration in order_by.split(","):
            if _order_by := declaration.strip():
                ...
                order_field, order_direction = parts[0], (
                    parts[1] if len(parts) > 1 else "desc"
                )
                order_direction = (
                    Order.asc if order_direction.lower() == "asc" else Order.desc
                )
    ...
    def parse(field: str, doctype: str):
        if "." in field:
            ...
            if field.startswith("`tab") or field.startswith('"tab'):
                _, child_doctype, child_field = re.search(
                    r'([`"])tab(.+?)\1.\1(.+)\1', field
                ).groups()
                ...
                if linked_field.fieldtype == "Link":
                    ...
                    return ChildTableField(
                        linked_doctype, fieldname, doctype, alias=alias
                    )
...
def get_nested_set_hierarchy_result(
    doctype: str, name: str, hierarchy: str
) -> list[str]:
