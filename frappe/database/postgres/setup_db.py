...
def setup_database():
    ...
    if root_conn.sql(f"SELECT 1 FROM pg_roles WHERE rolname='{frappe.conf.db_name}'"):
        root_conn.sql(
            f"ALTER USER \"{frappe.conf.db_name}\" WITH PASSWORD '{frappe.conf.db_password}'"
        )
    else:
        root_conn.sql(
            f"CREATE USER \"{frappe.conf.db_name}\" WITH PASSWORD '{frappe.conf.db_password}'"
        )
    root_conn.sql(f'CREATE DATABASE "{frappe.conf.db_name}"')
    root_conn.sql(
        f'GRANT ALL PRIVILEGES ON DATABASE "{frappe.conf.db_name}" TO "{frappe.conf.db_name}"'
    )
    if psql_version := root_conn.sql("SHOW server_version_num", as_dict=True):
        ...
        if cint(semver_version_num) > 150000:
            root_conn.sql(
                f'ALTER DATABASE "{frappe.conf.db_name}" OWNER TO "{frappe.conf.db_name}"'
            )
    root_conn.close()
...
def drop_user_and_database(db_name, root_login, root_password):
    root_conn = get_root_connection(
        frappe.flags.root_login or root_login,
        frappe.flags.root_password or root_password,
    )
    root_conn.commit()
    root_conn.sql(
