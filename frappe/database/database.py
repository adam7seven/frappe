...
class Database:
    ...
    def sql(
        ...
        *,
        ...
        if isinstance(query, MySQLQueryBuilder | PostgreSQLQueryBuilder):
            frappe.log(
                "Use run method to execute SQL queries generated by Query Builder"
            )
        ...
        if as_iterator:
            return self._return_as_iterator(
                pluck=pluck, as_dict=as_dict, as_list=as_list, update=update
            )
    ...
    def _return_as_iterator(self, *, pluck, as_dict, as_list, update):
        while result := self._transform_result(
            self._cursor.fetchmany(SQL_ITERATOR_BATCH_SIZE)
        ):
            if pluck:
                for row in result:
                    yield row[0]
                ...
                frappe.throw(
                    _("`as_iterator` only works with `as_list=True` or `as_dict=True`")
                )
    ...
    def mogrify(self, query: Query, values: QueryValues):
        ...
        try:
            ...
            if isinstance(values, dict):
                return query % {
                    k: frappe.db.escape(v) if isinstance(v, str) else v
    ...
    def get_values(
        ...
        *,
        ...
        if isinstance(filters, list):
            out = self._get_value_for_many_ids(
                doctype=doctype,
                ids=filters,
                field=fieldname,
                order_by=order_by,
            ...
            if (filters is not None) and (filters != doctype or doctype == "DocType"):
                try:
                    if order_by:
                        order_by = (
                            "modified" if order_by == DefaultOrderBy else order_by
                        )
    ...
    def set_single_value(
        ...
        *,
        ...
        frappe.db.delete(
            "Singles",
            filters={"field": ("in", tuple(to_update)), "doctype": doctype},
            debug=debug,
        )

        singles_data = (
            (doctype, key, sbool(value)) for key, value in to_update.items()
        )
    ...
    def bulk_update(
        ...
        *,
        ...
        for __ in range(0, total_docs, chunk_size):
            ...
            self._build_and_run_bulk_update_query(
                doctype, doc_chunk, modified_dict, debug
            )
    ...
    def _build_and_run_bulk_update_query(
        ...
        for field in conditions:
            ...
            update_query = update_query.set(
                dt[field], conditions[field].else_(dt[field])
            )
    ...
    def delete(
        self, doctype: str, filters: dict | list | None = None, debug=False, **kwargs
    ):
    ...
    def log_touched_tables(self, query):
        if is_query_type(
            query, ("insert", "delete", "update", "alter", "drop", "rename")
        ):
