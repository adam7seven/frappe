...
class DBTable:
    ...
    def validate(self):
        ...
        if self.meta.get("istable"):
            columns += [frappe._dict({"fieldname": f, "fieldtype": "Data"}) for f in frappe.db.CHILD_TABLE_COLUMNS]
...
def validate_column_name(n):
    if special_characters := SPECIAL_CHAR_PATTERN.findall(n):
        ...
        frappe.throw(
            _("Fieldname {0} cannot have special characters like {1}").format(frappe.bold(cstr(n)), special_characters),
