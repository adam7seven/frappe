...
class VirtualDoctypeTest(Document):
    ...
    def get_list(args):
        ...
        return [frappe._dict(doc) for id, doc in data.items()]
...
class TestVirtualDoctypes(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cdt = new_doctype(
            id=TEST_CHILD_DOCTYPE_NAME, is_virtual=1, istable=1, custom=0
        ).insert()
        vdt = new_doctype(
            id=TEST_DOCTYPE_NAME,
            is_virtual=1,
            custom=0,
            fields=[
                {
    ...
    def test_insert_update_and_load_from_desk(self):
        ...
        docid = frappe.response.docs[0]["id"]

        doc = frappe.get_doc(TEST_DOCTYPE_NAME, docid)

        doc.update(
            {"child_table": [{"id": "child-1", "some_fieldname": "child1-field-value"}]}
        )
    ...
    def test_multiple_doc_insert_and_get_list(self):
        ...
        doc1.append("child_table", {"id": "first", "some_fieldname": "first-value"})
        doc1.insert()
        doc2 = frappe.new_doc(doctype=TEST_DOCTYPE_NAME)
        doc2.append("child_table", {"id": "second", "some_fieldname": "second-value"})
        doc2.insert()
        ...
        doc2.reload()
        doc1.reload()
        updated_docs = {doc1.id, doc2.id}
        self.assertEqual(docs, updated_docs)

        listed_docs = {d.id for d in VirtualDoctypeTest.get_list({})}
    ...
    def test_delete_doc(self):
        ...
        frappe.delete_doc(doc.doctype, doc.id)

        listed_docs = {d.id for d in VirtualDoctypeTest.get_list({})}
        self.assertNotIn(doc.id, listed_docs)
