...
class TestClient(FrappeTestCase):
    def test_set_value(self):
        ...
        frappe.set_value("ToDo", todo.id, "description", "test 1")
        self.assertEqual(frappe.get_value("ToDo", todo.id, "description"), "test 1")

        frappe.set_value("ToDo", todo.id, {"description": "test 2"})
        self.assertEqual(frappe.get_value("ToDo", todo.id, "description"), "test 2")
    ...
    def test_delete(self):
        ...
        child_row_id = note.seen_by[0].id

        with patch.object(Note, "save") as save:
            delete("Note Seen By", child_row_id)
            save.assert_called()

        delete("Note", note.id)

        self.assertFalse(frappe.db.exists("Note", note.id))
        self.assertRaises(frappe.DoesNotExistError, delete, "Note", note.id)
        self.assertRaises(
            frappe.DoesNotExistError, delete, "Note Seen By", child_row_id
        )
    ...
    def test_client_insert(self):
        ...
        # cleanup
        frappe.delete_doc("Note", note1.id)
        frappe.delete_doc("Note", note2.id)
