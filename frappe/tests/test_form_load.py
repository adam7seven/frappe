...
class TestFormLoad(FrappeTestCase):
    def test_load(self):
        getdoctype("DocType")
        meta = next(filter(lambda d: d.id == "DocType", frappe.response.docs))
        self.assertEqual(meta.id, "DocType")
        self.assertTrue(meta.get("__js"))

        frappe.response.docs = []
        getdoctype("Event")
        meta = next(filter(lambda d: d.id == "Event", frappe.response.docs))
    ...
    def test_fieldlevel_permissions_in_load(self):
        ...
        self.assertEqual(blog_doc.id, blog.id)
        # since published field has higher permlevel
        with self.assertRaises(AttributeError):
            blog_doc.published
        ...
        doc = frappe.get_doc("Blog Post", blog.id)
        doc.published = 1
        doc.save()
        ...
        blog_doc.delete()
        frappe.delete_doc(
            blog_post_property_setter.doctype, blog_post_property_setter.id
        )
    ...
    def test_get_doc_info(self):
        ...
        # empty attachment
        save_file("test_file", b"", note.doctype, note.id, decode=True)
