...
class TestGlobalSearch(FrappeTestCase):
    ...
    def test_update_doc(self):
        ...
        event = frappe.get_doc("Event", frappe.get_all("Event")[0].id)
    ...
    def test_delete_doc(self):
        self.insert_test_events()
        event_id = frappe.get_all("Event")[0].id
        event = frappe.get_doc("Event", event_id)
        test_subject = event.subject
        results = global_search.search(test_subject)
        self.assertTrue(
            any(r["id"] == event_id for r in results),
            msg="Failed to search document by exact id",
        )
        ...
        results = global_search.search(test_subject)
        self.assertTrue(
            all(r["id"] != event_id for r in results),
    ...
    def test_insert_child_table(self):
        ...
        phrases = [
            ...
            'Its id means "male water snake", as opposed to Hydra, a much larger constellation that represents a female water snake. ',
