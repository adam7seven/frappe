...
class ThreadWithReturnValue(Thread):
    def __init__(
        self, group=None, target=None, name=None, args=(), kwargs=None, *, site=None
    ):
        if kwargs is None:
            kwargs = {}
    ...
    def run(self):
        if self._target is not None:
            with patch("frappe.app.get_site_name", return_value=self.site):
                header_patch = patch(
                    "frappe.get_request_header", new=patch_request_header
                )
...
class FrappeAPITestCase(IntegrationTestCase):
    ...
    def get(self, path: str, params: dict | None = None, **kwargs) -> TestResponse:
        return make_request(
            target=self.TEST_CLIENT.get, args=(path,), kwargs={"json": params, **kwargs}
        )

    def post(self, path, data, **kwargs) -> TestResponse:
        return make_request(
            target=self.TEST_CLIENT.post, args=(path,), kwargs={"json": data, **kwargs}
        )

    def put(self, path, data, **kwargs) -> TestResponse:
        return make_request(
            target=self.TEST_CLIENT.put, args=(path,), kwargs={"json": data, **kwargs}
        )

    def patch(self, path, data, **kwargs) -> TestResponse:
        return make_request(
            target=self.TEST_CLIENT.patch, args=(path,), kwargs={"json": data, **kwargs}
        )
...
class TestResourceAPI(FrappeAPITestCase):
    ...
    def test_get_list_debug(self):
        ...
        with suppress_stdout():
            response = self.get(
                self.resource(self.DOCTYPE), {"sid": self.sid, "debug": True}
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_run_doc_method(self):
        ...
        self.post(
            self.resource("Website Theme", "Standard"), {"run_method": "get_apps"}
        )
        response = self.get(
            self.resource("Website Theme", "Standard"), {"run_method": "get_apps"}
        )
        ...
        if response.status_code == 403:
            self.assertTrue(
                ...
                == {"exc_type", "exception", "exc", "_server_messages"}
            )
            self.assertEqual(response.json.get("exc_type"), "PermissionError")
            self.assertEqual(
                response.json.get("exception"),
...
class TestMethodAPI(FrappeAPITestCase):
    ...
    def test_get_user_info(self):
        ...
        self.assertIsInstance(response.json, dict)
        self.assertIn(
            response.json.get("message").get("user"), ("Administrator", "Guest")
        )
    ...
    def test_logs(self):
        ...
        def get_message(resp, msg_type):
            return frappe.parse_json(
                frappe.parse_json(frappe.parse_json(resp.json)[msg_type])[0]
            )
        ...
        self.assertEqual(
            get_message(response, "_server_messages").message, expected_message
        )
...
class TestWSGIApp(FrappeAPITestCase):
    def test_request_hooks(self):
        ...
        with self.patch_hooks(
            ...
            self.assertLess(
                _test_REQ_HOOK.get("before_request"),
                _test_REQ_HOOK.get("after_request"),
            )
...
class TestResponse(FrappeAPITestCase):
    def test_generate_pdf(self):
        response = self.get(
            ...
            {"sid": self.sid, "doctype": "User", "id": "Guest"},
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_binary_and_csv_response(self):
        ...
        encoded_filename = (
            filename.encode("utf-8").decode("unicode-escape", "ignore") + ".xlsx"
        )
        ...
        self.assertGreater(cint(response.headers["content-length"]), 0)
        self.assertEqual(
            response.headers["content-disposition"], f'filename="{encoded_filename}"'
        )
    ...
    def test_download_private_file_with_unique_url(self):
        ...
        self.assertEqual(
            self.get(file.unique_url, {"sid": self.sid}).text, test_content
        )
    ...
    def test_login_redirects(self):
        ...
        for redirect, expected_redirect in expected_redirects.items():
            response = self.get(
                f"/login?{urlencode({'redirect-to':redirect})}", {"sid": self.sid}
            )
