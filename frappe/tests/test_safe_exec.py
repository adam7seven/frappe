...
class TestSafeExec(FrappeTestCase):
    ...
    def test_sql(self):
        _locals = dict(out=None)
        safe_exec(
            """out = frappe.db.sql("select id from tabDocType where id='DocType'")""",
            None,
            _locals,
        )
    ...
    def test_query_builder(self):
        _locals = dict(out=None)
        safe_exec(
            script="""out = frappe.qb.from_("User").select(frappe.qb.terms.PseudoColumn("Max(id)")).run()""",
            _globals=None,
            _locals=_locals,
        )
        self.assertEqual(frappe.db.sql("SELECT Max(id) FROM tabUser"), _locals["out"])
