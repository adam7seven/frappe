...
class TestEmail(FrappeTestCase):
    ...
    def test_email_queue(self, send_after=None):
        ...
        email_queue = frappe.db.sql(
            """select id,message from `tabEmail Queue` where status='Not Sent'""",
            as_dict=1,
        )
    ...
    def test_send_after(self):
        ...
        flush()
        email_queue = frappe.db.sql(
            """select id from `tabEmail Queue` where status='Sent'""", as_dict=1
        )
    ...
    def test_cc_header(self):
        ...
        email_queue = frappe.db.sql(
            """select id from `tabEmail Queue` where status='Not Sent'""", as_dict=1
        )
        self.assertEqual(len(email_queue), 1)
        queue_recipients = [
            r.recipient
    ...
    def test_unsubscribe(self):
        ...
        unsubscribe(doctype="User", id="Administrator", email="test@example.com")
        self.assertTrue(
            frappe.db.get_value(
                "Email Unsubscribe",
                {
...
class TestEmailIntegrationTest(FrappeTestCase):
    ...
    def test_store_attachments(self):
        ...
        communication = frappe.get_doc("Communication", id)

        attachments = get_attachments(communication.doctype, communication.id)
        self.assertEqual(len(attachments), 1)

        file = frappe.get_doc("File", attachments[0].id)
            ...
            for part in email_content["parts"]:
                for child_part in part["childParts"]:
                    if child_part["isAttachment"]:
                        ...
                        self.assertIn("pdf", child_part["id"])
                        break
