...
class TestCustomFunctionsMariaDB(FrappeTestCase):
    ...
    def test_constant_column(self):
        query = frappe.qb.from_("DocType").select(
            "id", ConstantColumn("John").as_("User")
        )
        self.assertEqual(query.get_sql(), "SELECT `id`,'John' `User` FROM `tabDocType`")
    ...
    def test_cast(self):
        note = frappe.qb.DocType("Note")
        self.assertEqual("CONCAT(id,'')", Cast_(note.id, "varchar").get_sql())
        self.assertEqual("CAST(id AS INTEGER)", Cast_(note.id, "integer").get_sql())
        self.assertEqual(
            frappe.qb.from_("red")
            .from_(note)
            .select("other", Cast_(note.id, "varchar"))
            .get_sql(),
            "SELECT `tabred`.`other`,CONCAT(`tabNote`.`id`,'') FROM `tabred`,`tabNote`",
        )
...
class TestCustomFunctionsPostgres(FrappeTestCase):
    ...
    def test_constant_column(self):
        ...
        self.assertEqual(
            query.get_sql(), 'SELECT "id",\'John\' "User" FROM "tabDocType"'
        )
    ...
    def test_cast(self):
        note = frappe.qb.DocType("Note")
        self.assertEqual("CAST(id AS VARCHAR)", Cast_(note.id, "varchar").get_sql())
        self.assertEqual("CAST(id AS INTEGER)", Cast_(note.id, "integer").get_sql())
        self.assertEqual(
            frappe.qb.from_("red")
            .from_(note)
            .select("other", Cast_(note.id, "varchar"))
            .get_sql(),
            'SELECT "tabred"."other",CAST("tabNote"."id" AS VARCHAR) FROM "tabred","tabNote"',
        )
...
class TestBuilderBase:
    ...
    def test_agg_funcs(self):
        ...
        self.assertAlmostEqual(
            frappe.qb.avg("Communication", "rating"), 2.666, places=2
        )
...
class TestMisc(FrappeTestCase):
    ...
    def test_union(self):
        ...
        users = frappe.qb.from_(user).select(user.id)
        roles = frappe.qb.from_(role).select(role.id)
