...
class TestReportview(FrappeTestCase):
    ...
    def test_extract_fieldname(self):
        self.assertEqual(
            extract_fieldnames("count(distinct `tabPhoto`.id) as total_count")[0],
            "tabPhoto.id",
        )
        ...
        self.assertEqual(
            extract_fieldnames("count(`tabPhoto`.id) as total_count")[0], "tabPhoto.id"
        )

        self.assertEqual(
            extract_fieldnames("count(distinct `tabPhoto`.id)")[0], "tabPhoto.id"
        )

        self.assertEqual(extract_fieldnames("count(`tabPhoto`.id)")[0], "tabPhoto.id")

        self.assertEqual(
            extract_fieldnames("count(distinct `tabJob Applicant`.id) as total_count")[
                0
            ],
            "tabJob Applicant.id",
        )
        ...
        self.assertEqual(
            extract_fieldnames(
                "(1 / nullif(locate('a', `tabAddress`.`id`), 0)) as `_relevance`"
            )[0],
            "tabAddress.id",
        )
        ...
        self.assertEqual(
            extract_fieldnames("EXTRACT(MONTH FROM date_column) AS month")[0],
            "date_column",
        )
        ...
        self.assertEqual(
            extract_fieldnames("first_name + ' ' + last_name AS full_name"),
        ...
        self.assertEqual(
            extract_fieldnames(
                "CONCAT(id, '/', name, '/', age, '/', marks) AS student"
            ),
            ["id", "name", "age", "marks"],
        )

        self.assertEqual(
            extract_fieldnames("tablefield.fiedname")[0], "tablefield.fiedname"
        )

        self.assertEqual(
            extract_fieldnames("`tabChild DocType`.`fiedname`")[0],
            "tabChild DocType.fiedname",
        )
