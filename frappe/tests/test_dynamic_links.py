...
class TestDynamicLinks(IntegrationTestCase):
    ...
    def test_delete_with_comment(self):
        ...
        self.assertTrue(frappe.get_all("Comment", filters={"reference_doctype": "Event", "reference_id": event.name}))
        event.delete()
        self.assertFalse(frappe.get_all("Comment", filters={"reference_doctype": "Event", "reference_id": event.name}))
