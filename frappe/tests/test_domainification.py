...
class TestDomainification(FrappeTestCase):
    ...
    def tearDown(self):
        frappe.db.delete("Role", {"id": "_Test Role"})
        frappe.db.delete("Has Role", {"role": "_Test Role"})
        frappe.db.delete("Domain", {"id": ("in", ("_Test Domain 1", "_Test Domain 2"))})
    ...
    def test_doctype_and_role_domainification(self):
        ...
        self.assertTrue(
            "Test Domainification"
            not in [d.get("value") for d in results.get("doctypes")]
        )
        self.assertTrue(
            "_Test Role" not in [d.get("value") for d in results.get("roles")]
        )
    ...
    def test_desktop_icon_for_domainification(self):
        ...
        doctypes = [
            icon.get("_doctype")
            for icon in icons
            if icon.get("_doctype") == "Test Domainification"
