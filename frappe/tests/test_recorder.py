...
class TestQueryNormalization(FrappeTestCase):
    def test_query_normalization(self):
        test_cases = {
            "select * from user where id = 'x'": "select * from user where id = ?",
            "select * from user where a > 5": "select * from user where a > ?",
            "select * from `user` where a > 5": "select * from `user` where a > ?",
            "select `id` from `user`": "select `id` from `user`",
            "select `id` from `user` limit 10": "select `id` from `user` limit ?",
            "select `id` from `user` where id in ('a', 'b', 'c')": "select `id` from `user` where id in (?)",
        }
