...
class TestFixtureImport(FrappeTestCase):
    ...
    def insert_dummy_data_and_export(
        self, DocType: str, dummy_id_list: list[str]
    ) -> str:
        for id in dummy_id_list:
            doc = frappe.get_doc({"doctype": DocType, "member_name": id})
            doc.insert()
    ...
    def test_fixtures_import(self):
        ...
        self.assertEqual(frappe.db.count("temp_doctype"), len(dummy_id_list))
        ...
        self.assertEqual(set(dummy_id_list), imported_data)
    ...
    def test_singles_fixtures_import(self):
        ...
        dummy_id_list = ["Phoebe"]
        path_to_exported_fixtures = self.insert_dummy_data_and_export(
            "temp_singles", dummy_id_list
        )
        ...
        self.assertEqual(data, dummy_id_list[0])
