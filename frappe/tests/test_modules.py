...
class TestUtils(FrappeTestCase):
    ...
    def test_sync_customizations(self):
        with note_customizations() as (custom_field, property_setter):
            ...
            self.assertTrue(property_setter.doctype, property_setter.id)
            self.assertTrue(custom_prop_setter.doctype, custom_prop_setter.id)
    ...
    def test_export_doc(self):
        ...
        note.save()
        export_doc(doctype="Note", id=note.id)
        exported_doc_path = Path(
            frappe.get_app_path("frappe", "desk", "note", note.id, f"{note.id}.json")
        )
    ...
    def test_make_boilerplate(self):
        with temp_doctype() as doctype:
            scrubbed = frappe.scrub(doctype.id)
...
def temp_doctype():
    try:
        ...
        frappe.db.sql_ddl(f"DROP TABLE `tab{doctype.id}`")
