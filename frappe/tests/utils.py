...
class FrappeTestCase(unittest.TestCase):
    ...
    def enable_safe_exec(cls) -> None:
        ...
        update_site_config(
            SAFE_EXEC_CONFIG_KEY, 1, validate=False, site_config_path=cls._common_conf
        )
    ...
    def freeze_time(self, time_to_freeze, *args, **kwargs):
        ...
        fake_time_with_tz = timezone.localize(get_datetime(time_to_freeze)).astimezone(
            pytz.utc
        )
...
def check_orpahned_doctypes():
    ...
    doctypes = frappe.get_all("DocType", {"custom": 0}, pluck="id")
    orpahned_doctypes = []
