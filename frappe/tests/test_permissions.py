...
class TestPermissions(FrappeTestCase):
    ...
    def test_user_permissions_in_doc(self):
        add_user_permission(
            "Blog Category", "-test-blog-category-1", "test2@example.com"
        )
    ...
    def test_user_permissions_in_report(self):
        ...
        frappe.set_user("test2@example.com")
        ids = [
            d.id for d in frappe.get_list("Blog Post", fields=["id", "blog_category"])
        ]

        self.assertTrue("-test-blog-post-1" in ids)
        self.assertFalse("-test-blog-post" in ids)
    ...
    def test_user_link_match_report(self):
        ...
        ids = [d.id for d in frappe.get_list("Blog Post", fields=["id", "owner"])]
        self.assertTrue("-test-blog-post-2" in ids)
        self.assertFalse("-test-blog-post-1" in ids)
    ...
    def test_set_standard_fields_manually(self):
        ...
        fake_owner = frappe.db.get_value("User", {"id": ("!=", frappe.session.user)})
    ...
    def test_insert_if_owner_with_user_permissions(self):
        ...
        frappe.set_user("Administrator")
        remove_user_permission(
            "Blog Category", "-test-blog-category", "test2@example.com"
        )
    ...
    def test_contextual_user_permission(self):
        ...
        posts = frappe.get_all("Blog Post", fields=["id", "blogger"])
        ...
        posts = frappe.get_list("Blog Post", fields=["id", "blogger"])
    ...
    def test_if_owner_permission_on_getdoc(self):
        ...
        getdoc("Blog Post", doc.id)
        doclist = [d.id for d in frappe.response.docs]
        self.assertTrue(doc.id in doclist)

        frappe.set_user("test2@example.com")
        self.assertRaises(frappe.PermissionError, getdoc, "Blog Post", doc.id)
    ...
    def test_if_owner_permission_on_get_list(self):
        ...
        frappe.set_user("test2@example.com")
        self.assertIn(doc.id, frappe.get_list("Blog Post", pluck="id"))
        ...
        frappe.set_user("test2@example.com")
        self.assertNotIn(doc.id, frappe.get_list("Blog Post", pluck="id"))
    ...
    def test_if_owner_permission_on_delete(self):
        ...
        self.assertRaises(
            frappe.PermissionError, frappe.delete_doc, "Blog Post", doc.id
        )
    ...
    def test_clear_user_permissions(self):
        ...
        blog_post_user_permission_count = frappe.db.count(
            "User Permission",
            filters={"user": "test2@example.com", "allow": "Blog Post"},
        )
        ...
        blog_category_user_permission_count = frappe.db.count(
            "User Permission",
            filters={"user": "test2@example.com", "allow": "Blog Category"},
        )
    ...
    def test_child_permissions(self):
        ...
        self.assertIsInstance(
            frappe.get_list("DefaultValue", parent_doctype="User", limit=1), list
        )
    ...
    def test_select_user(self):
        ...
        frappe.set_user("test3@example.com")
        users = frappe.get_list("User", pluck="id")
    ...
    def test_get_doctypes_with_read(self):
        with self.set_user("Administrator"):
            doctype = (
                new_doctype(
                    permissions=[{"select": 1, "role": "_Test Role", "read": 0}]
                )
                .insert()
                .id
            )
