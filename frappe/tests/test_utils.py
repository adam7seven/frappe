...
class TestFilters(FrappeTestCase):
    ...
    def test_multiple_dict(self):
        self.assertTrue(
            evaluate_filters(
                {"doctype": "User", "status": "Open", "id": "Test 1"},
                {"status": "Open", "id": "Test 1"},
            )
        )
        self.assertFalse(
            evaluate_filters(
                {"doctype": "User", "status": "Open", "id": "Test 1"},
                {"status": "Closed", "id": "Test 1"},
            )
        )
...
class TestDiffUtils(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.doc = frappe.get_doc(
            doctype="Client Script", dt="Client Script", id="test_client_script"
        )
        cls.doc.insert()
        cls.doc.script = "2;"
        ...
        cls.versions = version_query(
            ...
            start=0,
            page_len=20,
            filters={"ref_doctype": cls.doc.doctype, "docid": cls.doc.id},
        )
...
class TestLinkTitle(FrappeTestCase):
        ...
        custom_doctype = frappe.get_doc(
            {
                ...
                "id": "Test Custom Doctype for Link Title",
            }
        )
    ...
    def test_link_titles_on_getdoc(self):
        ...
        self.assertTrue(f"{user.doctype}::{user.id}" in link_titles)
        self.assertEqual(link_titles[f"{user.doctype}::{user.id}"], user.full_name)
