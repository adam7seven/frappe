...
class TestAssign(IntegrationTestCase):
    ...
    def _test_basic_assign_on_document(self, doc):
        ...
        frappe.desk.form.assign_to.remove(doc.doctype, doc.id, "test@example.com")

        # assignment is cleared
        assignments = frappe.desk.form.assign_to.get(
            dict(doctype=doc.doctype, id=doc.id)
        )
    ...
    def test_assignment_count(self):
        ...
        data = {
            d.id: d.count for d in get_group_by_count(TEST_DOCTYPE, "[]", "assigned_to")
        }
        ...
        data = {
            d.id: d.count
            for d in get_group_by_count(TEST_DOCTYPE, '[{"public": 1}]', "assigned_to")
        }
    ...
    def test_assignment_removal(self):
        ...
        # remove assignment
        frappe.db.set_value("ToDo", new_todo[0].id, "allocated_to", "")

        self.assertFalse(get_assignments("ToDo", todo.id))
