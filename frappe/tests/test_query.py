...
class TestQuery(IntegrationTestCase):
    ...
    def test_string_fields(self):
        ...
        self.assertEqual(
            ...
            frappe.qb.from_("User").select(Field("name"), Field("email")).where(Field("name") == "Administrator").run(),
        )
    ...
    def test_functions_fields(self):
        ...
        self.assertEqual(
            ...
            frappe.qb.from_("User").select(Abs(Field("name") - Field("email")), Count(Field("name"))).get_sql(),
        )
        ...
        self.assertEqual(
            frappe.qb.get_query("User", fields="Count(name) as count, Max(email) as max_email", filters={}).get_sql(),
            frappe.qb.from_("User")
    ...
    def test_qb_fields(self):
        ...
        self.assertEqual(
            frappe.qb.get_query(user_doctype, fields=[user_doctype.name, user_doctype.email], filters={}).get_sql(),
    ...
    def test_aliasing(self):
        ...
        self.assertEqual(
            frappe.qb.get_query(user_doctype, fields=["Count(name) as count", "email as id"], filters={}).get_sql(),
            frappe.qb.from_(user_doctype)
    ...
    def test_comment_stripping(self):
        self.assertNotIn("email", frappe.qb.get_query("User", fields=["name", "#email"], filters={}).get_sql())
