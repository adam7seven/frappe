...
class TestDBUpdate(FrappeTestCase):
    def test_db_update(self):
        ...
        for field_def in field_defs:
            ...
            table_column = find(table_columns, lambda d: d.get("id") == fieldname)

            fieldtype = get_fieldtype_from_def(field_def)

            fallback_default = (
                "0"
                if field_def.get("fieldtype") in frappe.model.numeric_fieldtypes
                else "NULL"
            )
            default = (
                field_def.default if field_def.default is not None else fallback_default
            )

            self.assertEqual(fieldtype, table_column.type)
            self.assertIn(
                cstr(table_column.default) or "NULL", [cstr(default), f"'{default}'"]
            )
    ...
    def check_unique_indexes(self, doctype: str, field: str):
        ...
        self.assertEqual(
            len(indexes),
            1,
            msg=f"There should be 1 index on {doctype}.{field}, found {indexes}",
        )

    @run_only_if(db_type_is.MARIADB)  # postgres uses invalid type for <=15
    def test_bigint_conversion(self):
        doctype = new_doctype(
            fields=[{"fieldname": "int_field", "fieldtype": "Int"}]
        ).insert()

        with self.assertRaises(frappe.CharacterLengthExceededError):
            frappe.get_doc(doctype=doctype.id, int_field=2**62 - 1).insert()
    ...
    def test_unique_index_on_install(self):
        ...
        for dt in frappe.get_all(
            "DocType", {"is_virtual": 0, "issingle": 0}, pluck="id"
        ):
            doctype = frappe.get_meta(dt)
            fields = doctype.get("fields", filters={"unique": 1})
            for field in fields:
                with self.subTest(f"Checking index {doctype.id} - {field.fieldname}"):
                    self.check_unique_indexes(doctype.id, field.fieldname)
    ...
    def test_unique_index_on_alter(self):
        ...
        self.check_unique_indexes(doctype.id, field)
        ...
        doctype.save()
        self.check_unique_indexes(doctype.id, new_field.fieldname)
...
def get_table_column(doctype, fieldname):
    table_columns = frappe.db.get_table_columns_description(f"tab{doctype}")
    return find(table_columns, lambda d: d.get("id") == fieldname)
