...
class TestDB(IntegrationTestCase):
    ...
    def test_get_value(self):
        ...
        self.assertEqual(
            ...
            frappe.qb.from_(user_doctype).where(user_doctype.name == "Administrator").select("email").distinct().run(),
        )
    ...
    def test_bulk_update(self):
        ...
        updated_records = dict(
            frappe.get_all("ToDo", filters={"name": ("in", record_names)}, fields=["name", "description"], as_list=True)
        )
...
class TestDbConnectWithEnvCredentials(IntegrationTestCase):
    ...
    def test_connect_fails_with_wrong_credentials_by_env(self) -> None:
        ...
        with set_env_variable("FRAPPE_DB_PASSWORD", "pass"):
            ...
            self.assertTrue(re.search(r"(password authentication failed|Access denied for)", str(cm.exception)))
