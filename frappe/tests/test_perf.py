...
class TestPerformance(IntegrationTestCase):
    ...
    def test_set_value_query_count(self):
        ...
        with self.assertQueryCount(1):
            frappe.db.set_value("User", {"user_type": "System User"}, {"interest": "Nothing", "bio": "boring person"})
    ...
    def test_get_value_limits(self):
        ...
        for filter in filters:
            ...
            self.assertEqual(len(frappe.db.get_values("User", filters=filter)), frappe.db.count("User", filter))
    ...
    def test_no_ifnull_checks(self):
        query = frappe.get_all("DocType", {"autoid": ("is", "set")}, run=0).lower()
