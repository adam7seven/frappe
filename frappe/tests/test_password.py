...
class TestPassword(FrappeTestCase):
    ...
    def make_email_account(self, id="Test Email Account Password"):
        if not frappe.db.exists("Email Account", id):
            return frappe.get_doc(
                {
                    "doctype": "Email Account",
                    "domain": "example.com",
                    "email_account_name": id,
            ...
            return frappe.get_doc("Email Account", id)
    ...
    def test_hashed_password(self, user="test@example.com"):
        ...
        auth = get_password_list(dict(doctype="User", id=user))[0]
    ...
    def test_password_on_reid_user(self):
        password = "test-reid-password"
        ...
        old_id = doc.id
        new_id = old_id + "-new"
        frappe.reid_doc(doc.doctype, old_id, new_id)

        new_doc = frappe.get_doc(doc.doctype, new_id)
        self.assertEqual(new_doc.get_password(), password)
        self.assertTrue(not get_password_list(doc))

        frappe.reid_doc(doc.doctype, new_id, old_id)
