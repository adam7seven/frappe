...
class TestMethodAPIV2(FrappeAPITestCase):
    ...
    def test_shorthand_controller_methods(self):
        ...
        expanded_response = self.get(self.method("frappe.core.doctype.user.user.get_all_roles"), {"sid": self.sid})
    ...
    def test_logs(self):
        ...
        with suppress_stdout():
            response = self.get(self.method(method), {"sid": self.sid, "message": expected_message, "fail": True}).json
...
class TestReadOnlyMode(FrappeAPITestCase):
    ...
    def test_blocked_writes_v2(self):
        with suppress_stdout():
            response = self.post(self.resource("ToDo"), {"description": frappe.mock("paragraph"), "sid": self.sid})
