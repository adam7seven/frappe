    ...
    if filters_sql:
        try:
            coords = frappe.db.sql(
                f"""SELECT id, location FROM `tab{doctype}`  WHERE {filters_sql}""",
                as_dict=True,
            )
        except frappe.db.InternalError:
            frappe.msgprint(
                frappe._("This Doctype does not contain location fields"),
...
def return_coordinates(doctype, filters_sql):
    ...
    if filters_sql:
        try:
            ...
            frappe.msgprint(
                frappe._("This Doctype does not contain latitude and longitude fields"),
                raise_exception=True,
            )
            return
    else:
