...
def get_roles_and_doctypes():
    ...
    doctypes_list = [
        {"label": _(d.get("name")), "value": d.get("name")} for d in doctypes
    ]
    ...
    if role:
        ...
        if frappe.session.user != "Administrator":
            custom_roles = frappe.get_all(
                "Role", filters={"is_custom": 1}, pluck="name"
            )
        ...
        if not out:
            out = frappe.get_all(
                "DocPerm", fields="*", filters=filters, order_by="permlevel"
            )
...
def update(
    ...
    if ptype == "report" and value == "1" and if_owner == "1":
        frappe.throw(
            _("Cannot set 'Report' permission if 'Only If Creator' permission is set")
        )

    out = update_permission_property(
        doctype, role, permlevel, ptype, value, if_owner=if_owner
    )

    if ptype == "if_owner" and value == "1":
        update_permission_property(
            doctype, role, permlevel, "report", "0", if_owner=value
        )
