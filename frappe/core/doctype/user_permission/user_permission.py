...
def add_user_permissions(data):
    ...
    if data.apply_to_all_doctypes == 1 and not exists:
        ...
        insert_user_perm(data.user, data.doctype, data.docid, data.is_default, data.hide_descendants, apply_to_all=1)
        return 1
...
def insert_user_perm(user, doctype, docid, is_default=0, hide_descendants=0, apply_to_all=None, applicable=None):
