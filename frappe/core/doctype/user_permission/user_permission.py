...
class UserPermission(Document):
    ...
    def on_update(self):
        ...
        frappe.publish_realtime(
            "update_user_permissions", user=self.user, after_commit=True
        )
    ...
    def validate_user_permission(self):
        ...
        if duplicate_exists:
            frappe.throw(
                _("User permission already exists"), frappe.DuplicateEntryError
            )
    ...
    def validate_default_permission(self):
        ...
        if overlap_exists:
            ...
            frappe.throw(
                _("{0} has already assigned default value for {1}.").format(
                    ref_link, self.allow
                )
            )
