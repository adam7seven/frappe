...
class TestUserPermission(IntegrationTestCase):
    ...
    def test_default_user_permission_corectness(self):
        ...
        param = get_params(
            perm_user, "Blog Post", test_blog.id, is_default=1, hide_descendants=1
        )
        add_user_permissions(param)
    ...
    def test_for_applicable_on_update_from_apply_to_all(self):
        ...
        removed_apply_to_all = frappe.db.exists(
    ...
    def test_for_apply_to_all_on_update_from_applicable(self):
        ...
        is_created = add_user_permissions(param)
        is_created_apply_to_all = frappe.db.exists(
            "User Permission", get_exists_param(user)
        )
    ...
    def test_user_perm_for_nested_doctype(self):
        ...
        self.assertTrue(
            has_user_permission(frappe.get_doc("Person", parent_record.id), user.id)
        )
        self.assertTrue(
            has_user_permission(frappe.get_doc("Person", child_record.id), user.id)
        )
    ...
    def test_user_perm_on_new_doc_with_field_default(self):
        ...
        frappe.set_user("Administrator")
        remove_applicable(
            ["Assignment Rule"], "new_doc_test@example.com", "DocType", "ToDo"
        )
