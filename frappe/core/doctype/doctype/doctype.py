...
class DocType(Document):
    ...
    def validate_name(self, name=None):
        ...
        if len(name) > max_length:
            ...
            frappe.throw(
                _("Doctype name is limited to {0} characters ({1})").format(
                    max_length, name
                ),
                frappe.NameError,
            )
...
def validate_autoincrement_autoname(dt: Union[DocType, "CustomizeForm"]) -> bool:
    ...
    def get_autoname_before_save(dt: Union[DocType, "CustomizeForm"]) -> str:
        if dt.doctype == "Customize Form":
            property_value = frappe.db.get_value(
                "Property Setter",
                {"doc_type": dt.doc_type, "property": "autoname"},
                "value",
            )
    ...
    if not dt.is_new():
        ...
        if (is_autoname_autoincrement and autoname_before_save != "autoincrement") or (
            ...
            if dt.doctype == "Customize Form":
                frappe.throw(
                    _("Cannot change to/from autoincrement autoname in Customize Form")
                )
    ...
    def check_unique_fieldname(docname, fieldname):
        duplicates = list(
            filter(
                None,
                map(
                    lambda df: (df.fieldname == fieldname and str(df.idx)) or None,
                    fields,
                ),
            )
        )
...
def validate_permissions(doctype, for_remove=False, alert=False):
    ...
    def validate_permission_for_all_role(d):
        ...
        if doctype.custom:
            ...
            roles = [
                row.name for row in frappe.get_all("Role", filters={"is_custom": 1})
            ]
