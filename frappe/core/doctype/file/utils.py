...
def setup_folder_path(filename: str, new_parent: str) -> None:
    ...
    if file.is_folder:
        ...
        reid_doc(
            "File",
            file.id,
            file.get_id_based_on_parent_folder(),
...
def generate_file_name(
    id: str, suffix: str | None = None, is_private: bool = False
) -> str:
    ...
    def path_exists(id, is_private):
        return os.path.exists(encode(get_files_path(id, is_private=is_private)))

    if not path_exists(id, is_private):
        return id

    candidate_path = get_file_name(id, suffix)

    if path_exists(candidate_path, is_private):
        return generate_file_name(id, is_private=is_private)
