...
class TestFile(IntegrationTestCase):
    ...
    def tests_after_upload(self):
        ...
        file_folder = frappe.db.get_value("File", self.saved_id, "folder")
...
class TestFileUtils(IntegrationTestCase):
    def test_extract_images_from_doc(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "File", {"attached_to_id": todo.id, "is_private": is_private}
            )
        )
        self.assertRegex(todo.description, r"<img src=\"(.*)/files/pix\.png(.*)\">")
        self.assertListEqual(
            get_attached_images("ToDo", [todo.id])[todo.id], ["/private/files/pix.png"]
        )
