...
class TestExtensionValidations(IntegrationTestCase):
    ...
    def test_allowed_extension(self):
        ...
        bad_file = frappe.new_doc(
            "File", file_name=f"{file_name}.csv", content=content
        ).insert()
        frappe.db.rollback()
...
class TestFile(IntegrationTestCase):
    ...
    def test_default_folder(self):
        d = frappe.get_doc(
            {"doctype": "File", "file_name": _("Test_Folder"), "is_folder": 1}
        )
        d.save()
    ...
    def test_on_delete(self):
        ...
        self.assertEqual(
            frappe.db.get_value("File", _("Home/Test Folder 1"), "file_size"), 0
        )
    ...
    def test_file_url_validation(self):
        ...
        self.assertRaisesRegex(
            ValidationError,
            f"Cannot access file path {test_file.file_url}",
            test_file.validate,
        )
        ...
        self.assertRaisesRegex(
            ValidationError, "File name cannot have", test_file.validate
        )
...
class TestFileUtils(IntegrationTestCase):
    def test_extract_images_from_doc(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "File", {"attached_to_name": todo.name, "is_private": is_private}
            )
        )
        self.assertRegex(todo.description, r"<img src=\"(.*)/files/pix\.png(.*)\">")
        self.assertListEqual(
            get_attached_images("ToDo", [todo.name])[todo.name],
            ["/private/files/pix.png"],
        )
    ...
    def test_extract_images_from_comment(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "File", {"attached_to_name": test_doc.name, "is_private": is_private}
            )
        )
    ...
    def test_extract_images_from_communication(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "File",
                {"attached_to_name": communication.name, "is_private": is_private},
            )
        )
        self.assertRegex(
