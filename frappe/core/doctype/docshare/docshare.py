...
class DocShare(Document):
    ...
    def get_doc(self):
        if not getattr(self, "_doc", None):
            self._doc = frappe.get_doc(self.share_doctype, self.share_id)
        return self._doc
    ...
    def check_share_permission(self):
        if not self.flags.ignore_share_permission and not frappe.has_permission(
            ...
            frappe.throw(
                _('You need to have "Share" permission'), frappe.PermissionError
            )
    ...
    def check_is_submittable(self):
        if self.submit and not cint(
            ...
            frappe.throw(
                ...
                ).format(frappe.bold(self.share_id), frappe.bold(self.share_doctype))
            )
    ...
    def after_insert(self):
        ...
        if self.everyone:
            doc.add_comment(
                "Shared", _("{0} shared this document with everyone").format(owner)
            )
        else:
            doc.add_comment(
                "Shared",
