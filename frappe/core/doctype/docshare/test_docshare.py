...
class TestDocShare(IntegrationTestCase):
    ...
    def test_add(self):
        # user not shared
        self.assertTrue(
            self.event.id not in frappe.share.get_shared("Event", self.user)
        )
    ...
    def test_permission_to_share(self):
        frappe.set_user(self.user)
        self.assertRaises(
            frappe.PermissionError, frappe.share.add, "Event", self.event.id, self.user
        )
    ...
    def test_share_with_everyone(self):
        ...
        self.assertTrue(self.event.id in frappe.share.get_shared("Event", self.user))
        self.assertTrue(
            self.event.id in frappe.share.get_shared("Event", "test1@example.com")
        )
        self.assertTrue(self.event.id not in frappe.share.get_shared("Event", "Guest"))

        frappe.share.set_permission(
            "Event", self.event.id, None, "read", value=0, everyone=1
        )
        self.assertTrue(
            self.event.id not in frappe.share.get_shared("Event", self.user)
        )
        self.assertTrue(
            self.event.id not in frappe.share.get_shared("Event", "test1@example.com")
        )
        self.assertTrue(self.event.id not in frappe.share.get_shared("Event", "Guest"))
    ...
    def test_share_with_submit_perm(self):
        ...
        submittable_doc = frappe.get_doc(
            doctype=doctype, test="test docshare with submit"
        ).insert()
        ...
        self.assertTrue(
            frappe.has_permission(
                doctype, "submit", doc=submittable_doc.id, user=self.user
            )
        )
        ...
        self.assertTrue(
            frappe.has_permission(
                doctype, "read", doc=submittable_doc.id, user=self.user
            )
        )
        self.assertTrue(
            frappe.has_permission(
                doctype, "write", doc=submittable_doc.id, user=self.user
            )
        )
    ...
    def test_share_int_pk(self):
        ...
        self.assertIn(
            str(test_doc.id),
            [str(id) for id in frappe.get_list("Console Log", pluck="id")],
        )
    ...
    @IntegrationTestCase.change_settings(
        "System Settings", {"disable_document_sharing": 1}
    )
