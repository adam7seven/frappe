...
class TestDocShare(IntegrationTestCase):
    ...
    def test_add(self):
        # user not shared
        self.assertTrue(
            self.event.name not in frappe.share.get_shared("Event", self.user)
        )
    ...
    def test_permission_to_share(self):
        ...
        frappe.set_user(self.user)
        frappe.share.add(
            "Event", self.event.name, "test1@example.com", write=1, share=1
        )
    ...
    def test_share_with_everyone(self):
        ...
        self.assertTrue(
            self.event.name not in frappe.share.get_shared("Event", "Guest")
        )
    ...
    def test_share_with_submit_perm(self):
        ...
        self.assertTrue(
            frappe.has_permission(
                doctype, "submit", doc=submittable_doc.name, user=self.user
            )
        )
        ...
        self.assertTrue(
            frappe.has_permission(
                doctype, "read", doc=submittable_doc.name, user=self.user
            )
        )
        self.assertTrue(
            frappe.has_permission(
                doctype, "write", doc=submittable_doc.name, user=self.user
            )
        )
    ...
    def test_share_int_pk(self):
        ...
        self.assertIn(
            str(test_doc.name),
            [str(name) for name in frappe.get_list("Console Log", pluck="name")],
        )
