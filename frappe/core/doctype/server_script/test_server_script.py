...
class TestServerScript(FrappeTestCase):
    ...
    def test_commit_in_doctype_event(self):
        ...
        self.assertRaises(
            AttributeError,
            frappe.get_doc(dict(doctype="ToDo", description="test me")).insert,
        )
...
"""
    ...
    def test_server_script_scheduled(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "Scheduled Job Type", {"server_script": scheduled_script.id}
            )
        )

        cron_job_id = frappe.db.get_value(
            "Scheduled Job Type", {"server_script": cron_script.id}
        )
        self.assertTrue(cron_job_id)

        cron_job = frappe.get_doc("Scheduled Job Type", cron_job_id)
