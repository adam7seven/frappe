...
class TestServerScript(IntegrationTestCase):
    ...
    def test_doctype_event(self):
        ...
        role = frappe.get_doc(doctype="Role", role_name="_Test Role 9").insert(
    ...
    def test_api_return(self):
        self.assertEqual(
            frappe.get_doc("Server Script", "test_return_value").execute_method(),
            "hello",
        )
...
"""
    ...
    def test_server_script_scheduled(self):
        ...
        self.assertTrue(
            frappe.db.exists(
                "Scheduled Job Type", {"server_script": scheduled_script.name}
            )
        )

        cron_job_name = frappe.db.get_value(
            "Scheduled Job Type", {"server_script": cron_script.name}
        )
        ...
        updated_cron_job_name = frappe.db.get_value(
            "Scheduled Job Type", {"server_script": cron_script.name}
        )
    ...
    def test_server_script_state_changes(self):
        ...
        job: ScheduledJobType = frappe.get_doc(
            "Scheduled Job Type", {"server_script": script.name}
        )
