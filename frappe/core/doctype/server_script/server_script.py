    ...
    def sync_scheduled_job_type(self):
        ...
        def get_scheduled_job() -> "ScheduledJobType":
            if scheduled_script := frappe.db.get_value(
                "Scheduled Job Type", {"server_script": self.name}
            ):
                return frappe.get_doc("Scheduled Job Type", scheduled_script)
            else:
                return frappe.get_doc(
                    {"doctype": "Scheduled Job Type", "server_script": self.name}
                )
        ...
        frappe.msgprint(
            _("Scheduled execution for script {0} has updated").format(self.name),
            alert=True,
        )
