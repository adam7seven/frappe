...
class UserType(Document):
    ...
    def on_trash(self):
        if self.is_standard:
            frappe.throw(
                _("Standard user type {0} can not be deleted.").format(
                    frappe.bold(self.id)
                )
            )
    ...
    def validate_document_type_limit(self):
        limit = frappe.conf.get("user_type_doctype_limit", {}).get(
        ...
        custom_doctypes = [
            row.document_type for row in self.user_doctypes if row.is_custom
        ]
    ...
    def add_select_perm_doctypes(self):
        ...
        for doctype in user_doctypes:
            ...
            for child_table in doc.get_table_fields():
                ...
                if child_doc:
                    self.prepare_select_perm_doctypes(
                        child_doc, user_doctypes, select_doctypes
                    )
    ...
    def add_role_permissions_for_file(self):
        ...
        frappe.db.set_value(
            "Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1}
        )
...
def add_role_permissions(doctype, role):
    id = frappe.get_value(
        "Custom DocPerm", dict(parent=doctype, role=role, permlevel=0)
    )
...
def user_linked_with_permission_on_doctype(doc, user):
    ...
    if not doc.user_id_field:
        frappe.throw(
            _("User Id Field is mandatory in the user type {0}").format(
                frappe.bold(doc.id)
            )
        )

    if frappe.db.get_value(
        doc.apply_user_permission_on, {doc.user_id_field: user}, "id"
    ):
        return True
    else:
        label = (
            frappe.get_meta(doc.apply_user_permission_on)
            .get_field(doc.user_id_field)
            .label
        )
    ...
    for user_type, data in user_types.items():
        ...
        if doc.get(data[1]) and (
            ...
            or not frappe.db.get_value(
                "User Permission",
                {"user": doc.get(data[1]), "allow": data[0], "for_value": doc.id},
                "id",
            )
        ):
            perm_data = frappe.db.get_value(
                "User Permission",
                {"allow": doc.doctype, "for_value": doc.id},
                ["id", "user"],
            )
            ...
            if not perm_data:
                ...
                frappe.db.set_value(
                    "User Permission", perm_data[0], "user", doc.get(data[1])
                )
