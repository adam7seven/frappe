...
class UserType(Document):
    ...
    def validate_document_type_limit(self):
        ...
        if not limit:
            frappe.throw(
                _("The limit has not set for the user type {0} in the site config file.").format(frappe.bold(self.id)),
                title=_("Set Limit"),
            )
    ...
    def validate_role(self):
        ...
        if not frappe.db.get_value("Role", self.role, "is_custom"):
            frappe.throw(
                _("The role {0} should be a custom role.").format(frappe.bold(get_link_to_form("Role", self.role)))
            )
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
    ...
    custom_doctypes = frappe.get_all("Custom Field", fields=["dt as id"], filters=custom_dt_filters, as_list=1)
...
def user_linked_with_permission_on_doctype(doc, user):
    ...
    if frappe.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "id"):
        ...
        frappe.msgprint(
            _("To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record.").format(
