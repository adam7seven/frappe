...
class User(Document):
    ...
    def set_roles_and_modules_based_on_user_type(self):
        ...
        if user_type_doc.role:
            ...
            if user_linked_with_permission_on_doctype(user_type_doc, self.id):
                ...
                frappe.msgprint(_("Role has been set as per the user type {0}").format(self.user_type), alert=True)
    ...
    def send_password_notification(self, new_password):
        try:
            ...
            frappe.msgprint(_("Please setup default outgoing Email Account from Settings > Email Account"), alert=True)
...
def _get_user_for_update_password(key, old_password):
    ...
    if key:
        ...
        if result.user:
            reset_password_link_expiry = cint(frappe.get_system_settings("reset_password_link_expiry_duration"))
