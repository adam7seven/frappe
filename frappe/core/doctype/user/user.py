...
class User(Document):
    ...
    def populate_role_profile_roles(self):
        if self.role_profile_id:
            role_profile = frappe.get_doc("Role Profile", self.role_profile_id)
            self.set("roles", [])
    ...
    def check_enable_disable(self):
        ...
        toggle_notifications(self.id, enable=cint(self.enabled))
    ...
    def send_password_notification(self, new_password):
        try:
            ...
            frappe.msgprint(
                _("Please setup default outgoing Email Account from Settings > Email Account"),
                alert=True,
            )
    ...
    def on_trash(self):
        ...
        (frappe.qb.update(todo_table).set(todo_table.assigned_by, None).where(todo_table.assigned_by == self.id)).run()
        ...
        frappe.db.delete(
            ...
            filters=(
                ...
                & ((table.reference_id == self.id) | table.owner == self.id)
            ),
            run=False,
        )
        # unlink contact
        ...
        # Delete EPS data
        frappe.db.delete("Energy Point Log", {"user": self.id})
    ...
    def before_reid(self, old_id, new_id, merge=False):
        # if merging, delete the old user notification settings
        if merge:
            frappe.delete_doc("Notification Settings", old_id, ignore_permissions=True)

        frappe.clear_cache(user=old_id)
        self.validate_reid(old_id, new_id)

    def validate_reid(self, old_id, new_id):
        # do not allow renaming administrator and guest
        if old_id in STANDARD_USERS:
            throw(_("User {0} cannot be reided").format(self.id))

        self.validate_email_type(new_id)
    ...
    def after_reid(self, old_id, new_id, merge=False):
        ...
        if frappe.db.exists("Notification Settings", old_id):
            frappe.reid_doc(
        ...
        # set email
        frappe.db.set_value("User", new_id, "email", new_id)
    ...
    def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
        ...
        login_with_mobile = cint(frappe.db.get_single_value("System Settings", "allow_login_using_mobile_number"))
...
def test_password_strength(new_password: str, key=None, old_password=None, user_data: tuple | None = None):
    ...
    if key is not None or old_password is not None:
        deprecation_warning("Arguments `key` and `old_password` are deprecated in function `test_password_strength`.")
...
def _get_user_for_update_password(key, old_password):
    ...
    if key:
        ...
        user = frappe.db.get_value(
            ...
            ["id", "last_reset_password_key_generated_on"],
        )
