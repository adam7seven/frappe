...
class TestUser(IntegrationTestCase):
    ...
    def test_delete(self):
        ...
        self.assertTrue(not frappe.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",)))
    ...
    def test_get_value(self):
        ...
        self.assertEqual(
            frappe.db.get_value("User", {"email": "test@example.com", "first_name": "_Test"}, ["first_name", "email"]),
            ("_Test", "test@example.com"),
        )

        test_user = frappe.db.sql("select * from tabUser where id='test@example.com'", as_dict=True)[0]
        self.assertEqual(frappe.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user)
...
@contextmanager
def test_user(*, first_name: str | None = None, email: str | None = None, roles: list[str], commit=False, **kwargs):
    try:
