...
class TestUser(IntegrationTestCase):
    ...
    def test_delete(self):
        ...
        self.assertTrue(
            not frappe.db.sql(
                """select * from `tabToDo` where allocated_to=%s""",
                ("_test@example.com",),
            )
        )
    ...
    def test_password_strength(self):
        ...
        with change_settings(
            "System Settings", enable_password_policy=1, minimum_password_score=2
        ):
    ...
    @IntegrationTestCase.change_settings(
        "System Settings", commit=True, password_reset_limit=1
    )
    ...
    def test_user_reid(self):
        old_id = "test_user_reid@example.com"
        new_id = "test_user_reid_new@example.com"
        user = frappe.get_doc(
            {
                "doctype": "User",
                "email": old_id,
        ...
        frappe.reid_doc("User", user.id, new_id)
        self.assertTrue(frappe.db.exists("Notification Settings", new_id))

        frappe.delete_doc("User", new_id)
    ...
    @IntegrationTestCase.change_settings(
        "System Settings", reset_password_link_expiry_duration=1
    )
