...
class Communication(Document, CommunicationEmailMixin):
        ...
        if self.reference_doctype and self.reference_id:
            if not self.reference_owner:
                self.reference_owner = frappe.db.get_value(
                    self.reference_doctype, self.reference_id, "owner"
                )
    ...
    def after_insert(self):
        ...
        if self.communication_type == "Communication":
            ...
            if self.reference_id == frappe.session.user:
                message = self.as_dict()
                message["broadcast"] = True
                frappe.publish_realtime("new_message", message, after_commit=True)
            else:
                # reference_id contains the user who is addressed in the messages' page comment
                frappe.publish_realtime(
                    "new_message",
                    self.as_dict(),
                    user=self.reference_id,
                    after_commit=True,
                )
    ...
    def set_delivery_status(self, commit=False):
        ...
        status_counts = Counter(
            frappe.get_all(
                "Email Queue", pluck="status", filters={"communication": self.id}
            )
        )
    ...
    def deduplicate_timeline_links(self):
        ...
        unique_links = {
            (link.link_doctype, link.link_id) for link in self.timeline_links
        }
        self.timeline_links = []
    ...
    def add_link(self, link_doctype, link_id, autosave=False):
        self.append(
            "timeline_links", {"link_doctype": link_doctype, "link_id": link_id}
        )
    ...
    def remove_link(
        self, link_doctype, link_id, autosave=False, ignore_permissions=True
    ):
        ...
        if not contact_id and email and auto_create_contact:
            ...
            try:
                ...
                contact = frappe.get_doc(
                    {"doctype": "Contact", "first_name": contact_id, "id": contact_id}
                )
...
def parse_email(email_strings):
    ...
    a doctype and docid ie in the format `admin+doctype+docname@example.com` or `admin+doctype=docname@example.com`,
    ...
    for email_string in email_strings:
        ...
        for email in email_string.split(","):
            ...
            if len(email_local_parts) == 3:
                ...
                docid = unquote(email_local_parts[2])
                ...
                docid = unquote(document_parts[1])
