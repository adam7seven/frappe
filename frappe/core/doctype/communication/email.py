...
def make(
    ...
    **kwargs,
    ...
    if (
        doctype
        and id
        and not frappe.has_permission(doctype=doctype, ptype="email", doc=id)
    ):
        raise frappe.PermissionError(
            f"You are not allowed to send emails related to: {doctype} {id}"
        )
    ...
    for a in attachments:
        if isinstance(a, str):
            attach = frappe.db.get_value(
                "File", {"id": a}, ["file_url", "is_private"], as_dict=1
            )
            file_args = {
...
def update_communication_as_read(id):
    ...
    communication = frappe.db.get_value(
        "Communication", id, "read_by_recipient", as_dict=True
    )
