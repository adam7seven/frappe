...
class TestReport(FrappeTestCase):
    ...
    def test_report_builder(self):
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "user_activity_report.json")
        ) as f:
    ...
    def test_custom_report(self):
        ...
        admin_dict = frappe.core.utils.find(
            result, lambda d: d["id"] == "Administrator"
        )
    ...
    def test_report_permissions(self):
        frappe.set_user("test@example.com")
        frappe.db.delete(
            "Has Role", {"parent": frappe.session.user, "role": "Test Has Role"}
        )
        frappe.db.commit()
...
"""
    ...
    def test_cte_in_query_report(self):
        ...
        if frappe.db.db_type == "mariadb":
            ...
            self.assertRaises(
                frappe.PermissionError, report.execute_query_report, filters={}
            )
