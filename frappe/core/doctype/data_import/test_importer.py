...
class TestImporter(IntegrationTestCase):
    ...
    def test_data_validation_semicolon_success(self):
        ...
        data_import = self.get_importer(
            doctype_id, import_file, update=True, use_sniffer=True
        )
    ...
    def test_data_validation_semicolon_failure(self):
        ...
        data_import = self.get_importer_semicolon(
            doctype_id, import_file, use_sniffer=True
        )
        doc = data_import.get_preview_from_template().get("data", [{}])
    ...
    def test_data_import_without_mandatory_values(self):
        ...
        self.assertEqual(
            frappe.parse_json(frappe.parse_json(import_log[0]["messages"])[0])[
                "message"
            ],
            expected_error,
        )
        ...
        self.assertEqual(
            frappe.parse_json(frappe.parse_json(import_log[0]["messages"])[1])[
                "message"
            ],
            expected_error,
        )
        ...
        self.assertEqual(
            frappe.parse_json(frappe.parse_json(import_log[1]["messages"])[0])[
                "message"
            ],
            "Title is required",
        )
    ...
    def test_data_import_update(self):
        ...
        self.assertEqual(
            updated_doc.table_field_1[0].id, existing_doc.table_field_1[0].id
        )
        self.assertEqual(
    ...
    def get_importer_semicolon(
        self, doctype, import_file, update=False, use_sniffer=False
    ):
        data_import = frappe.new_doc("Data Import")
        data_import.import_type = (
            "Insert New Records" if not update else "Update Existing Records"
...
def create_doctype_if_not_exists(doctype_id, force=False):
    ...
    # Child Table 2
    table_2_id = "Child 2 of " + doctype_id
    frappe.get_doc(
        {
            "doctype": "DocType",
            "id": table_2_id,
