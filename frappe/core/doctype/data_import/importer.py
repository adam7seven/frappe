...
class Importer:
    ...
    def export_import_log(self):
        ...
        for log in import_log:
            ...
            message = "Successfully Imported {}".format(log.get("docid")) if log.get("success") else log.get("messages")
    ...
    def parse(self):
        ...
        if self.map_to_field and self.map_to_field != "Don't Import":
            ...
            if df:
                ...
                self.warnings.append(
                    {
                        "message": _("Could not map column {0} to field {1}").format(column_number, self.map_to_field),
                        "type": "info",
                    }
                )
        else:
    ...
    def validate_values(self):
        ...
        if self.df.fieldtype == "Link1":
            ...
            if not self.date_format:
                ...
                message = _("{0} format could not be determined from the values in this column. Defaulting to {1}.")
                self.warnings.append(
                    {
                        "col": self.column_number,
