...
class TestScheduledJobType(IntegrationTestCase):
    ...
    def test_sync_jobs(self):
        ...
        daily_job = frappe.get_doc(
            "Scheduled Job Type",
            dict(method="frappe.desk.notifications.clear_notifications"),
        )
    ...
    def test_weekly_job(self):
        ...
        job.db_set("last_execution", "2019-01-01 00:00:00")
        self.assertTrue(
            job.is_event_due(get_datetime("2019-01-06 00:10:01"))
        )  # +10 min because of jitter
    ...
    def test_monthly_job(self):
        job = frappe.get_doc(
            "Scheduled Job Type",
            dict(
                method="frappe.email.doctype.auto_email_report.auto_email_report.send_monthly"
            ),
        )
        job.db_set("last_execution", "2019-01-01 00:00:00")
