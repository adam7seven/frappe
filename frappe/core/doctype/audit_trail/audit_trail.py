...
class AuditTrail(Document):
    ...
    def validate(self):
        self.validate_doctype_id()
        self.validate_document()

    def validate_doctype_id(self):
        if not self.doctype_id:
    ...
    def compare_document(self):
        ...
        self.amended_docs = [
            frappe.get_doc(self.doctype_id, id) for id in amended_document_ids
        ]
        ...
        for i in range(1, self.docs_to_compare):
            diff = get_diff(
                self.amended_docs[i - 1], self.amended_docs[i], compare_cancelled=True
            )
    ...
    def get_amended_documents(self):
        ...
        while curr_doc and len(amended_document_ids) < 5:
    ...
    def get_rows_updated_grid(self, i, diff):
        for change in diff.row_changed:
            ...
            for field in change[-1]:
                fieldname = get_field_label(
                    field[0], doctype=self.doctype_id, child_field=change[0]
                )
                value = field[-1]
                value_list = [""] * self.docs_to_compare
