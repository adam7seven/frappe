...
class TestAuditTrail(FrappeTestCase):
    ...
    def test_compare_changed_fields(self):
        ...
        comparator = create_comparator_doc(
            "Test Custom Doctype for Doc Comparator", re_amended_doc.id
        )
        documents, results = comparator.compare_document()

        test_field_values = results["changed"]["Field"]
        self.check_expected_values(
            test_field_values, ["first value", "second value", "third value"]
        )
    ...
    def test_compare_rows(self):
        ...
        child_table_new = [
            {"test_table_field": "new row 1 value"},
            {"test_table_field": "row 2 value"},
        ]
        rows_updated = frappe._dict(child_table_field=child_table_new)
        amended_doc = amend_document(doc, {}, rows_updated, 1)

        comparator = create_comparator_doc(
            "Test Custom Doctype for Doc Comparator", amended_doc.id
        )
        documents, results = comparator.compare_document()
    ...
    def check_rows_updated(self, row_changed):
        ...
        self.check_expected_values(
            table_field_values, ["old row 1 value", "new row 1 value"]
        )
...
def create_comparator_doc(doctype_id, document):
    ...
    comparator.doctype_id = doctype_id
