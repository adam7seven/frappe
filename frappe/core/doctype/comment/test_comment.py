...
class TestComment(IntegrationTestCase):
    def test_comment_creation(self):
        ...
        comment_1 = frappe.get_all(
            ...
            fields=["*"],
            filters=dict(
                reference_doctype=test_doc.doctype, reference_name=test_doc.name
            ),
        )[0]
    ...
    def test_public_comment(self):
        ...
        self.assertEqual(
            frappe.get_all(
                ...
                fields=["*"],
                filters=dict(
                    reference_doctype=test_blog.doctype, reference_name=test_blog.name
                ),
            )[0].published,
            1,
        )
        ...
        add_comment_args.update(
            comment="<script>alert(1)</script>Comment", comment_by="hacker"
        )
        add_comment(**add_comment_args)
        self.assertEqual(
            frappe.get_all(
    ...
    def test_user_not_logged_in(self):
        some_system_user = frappe.db.get_value(
            "User", {"name": ("not in", frappe.STANDARD_USERS)}
        )
