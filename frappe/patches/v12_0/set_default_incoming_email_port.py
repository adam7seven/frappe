...
def setup_incoming_email_port_in_email_domains():
    ...
    for domain in email_domains:
        if not domain.incoming_port:
            ...
            frappe.db.set_value("Email Domain", domain.id, "incoming_port", incoming_port, update_modified=False)
...
def setup_incoming_email_port_in_email_accounts():
    email_accounts = frappe.get_all("Email Account", ["incoming_port", "use_imap", "use_ssl", "id", "enable_incoming"])

    for account in email_accounts:
        if account.enable_incoming and not account.incoming_port:
            incoming_port = get_port(account)
            frappe.db.set_value("Email Account", account.id, "incoming_port", incoming_port, update_modified=False)
