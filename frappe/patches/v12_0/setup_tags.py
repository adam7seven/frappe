...
def execute():
    ...
    for doctype in frappe.get_list(
        "DocType", filters={"istable": 0, "issingle": 0, "is_virtual": 0}
    ):
        if not frappe.db.count(doctype.id) or not frappe.db.has_column(
            doctype.id, "_user_tags"
        ):
            continue

        for _user_tags in frappe.db.sql(
            f"select `id`, `_user_tags` from `tab{doctype.id}`", as_dict=True
        ):
            if not _user_tags.get("_user_tags"):
                continue

            for tag in (
                _user_tags.get("_user_tags").split(",")
                if _user_tags.get("_user_tags")
                else []
            ):
                if not tag:
                    continue
