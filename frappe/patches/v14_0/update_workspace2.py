...
def create_content(doc):
    ...
    if doc.get("onboarding"):
        content.append(
            {
                "type": "onboarding",
                "data": {"onboarding_name": doc.onboarding, "col": 12},
            }
        )
    if doc.charts:
        invalid_links = []
        for c in doc.charts:
            if c.get_invalid_links()[0]:
                ...
                content.append(
                    {"type": "chart", "data": {"chart_name": c.label, "col": 12}}
                )
    ...
    if doc.shortcuts:
        ...
        content.append(
            {
                "type": "header",
                "data": {
                    "text": doc.get("shortcuts_label") or _("Your Shortcuts"),
                    "level": 4,
                    "col": 12,
                },
            }
        ...
        for s in doc.shortcuts:
            if s.get_invalid_links()[0]:
                ...
                content.append(
                    {"type": "shortcut", "data": {"shortcut_name": s.label, "col": 4}}
                )
        for l in invalid_links:
    ...
    if doc.links:
        ...
        content.append(
            {
                "type": "header",
                "data": {
                    "text": doc.get("cards_label") or _("Reports & Masters"),
                    "level": 4,
                    "col": 12,
                },
            }
        ...
        for l in doc.links:
            if l.type == "Card Break":
                content.append(
                    {"type": "card", "data": {"card_name": l.label, "col": 4}}
                )
