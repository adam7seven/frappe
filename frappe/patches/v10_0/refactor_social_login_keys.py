...
def run_patch():
    ...
    users = frappe.get_all(
        "User", fields=["*"], filters={"id": ("not in", ["Administrator", "Guest"])}
    )
    ...
    for user in users:
        ...
        if user.frappe_userid:
            insert_user_social_login(
                user.id, user.modified_by, "frappe", idx, userid=user.frappe_userid
            )
            idx += 1
        ...
        if user.google_userid:
            insert_user_social_login(
                user.id, user.modified_by, "google", idx, userid=user.google_userid
            )
            idx += 1
