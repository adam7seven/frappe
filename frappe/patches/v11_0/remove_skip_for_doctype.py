...
def execute():
    ...
    for user_permission in frappe.get_all("User Permission", fields=["*"]):
        ...
        if has_skip_for_doctype:
            ...
            if (
                skip_for_doctype_map.get((user_permission.allow, user_permission.user))
                is None
            ):
                ...
                skip_for_doctype_map[(user_permission.allow, user_permission.user)] = (
                    skip_for_doctype
                )
            else:
        ...
        if skip_for_doctype:
            ...
            frappe.db.set_value(
                "User Permission", user_permission.id, "apply_to_all_doctypes", 1
            )
    ...
    if user_permissions_to_delete:
        frappe.db.delete(
            "User Permission",
            filters=(Field("id").isin(tuple(user_permissions_to_delete))),
        )
