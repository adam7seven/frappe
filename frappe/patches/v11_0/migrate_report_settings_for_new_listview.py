...
def execute():
    ...
    for report_id in reports:
        settings = frappe.db.get_value("Report", report_id, "json")
        if not settings:
            continue
        ...
        settings.order_by = (
            (settings.sort_by or "creation") + " " + (settings.order_by or "desc")
        )
        ...
        frappe.db.set_value("Report", report_id, "json", json.dumps(settings))
