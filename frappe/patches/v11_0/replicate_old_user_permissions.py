...
def execute():
    ...
    for permission in user_permissions:
        if (permission.allow, permission.user) not in doctype_to_skip_map:
            doctype_to_skip_map[(permission.allow, permission.user)] = (
                get_doctypes_to_skip(permission.allow, permission.user)
            )
...
def get_doctypes_to_skip(doctype, user):
    ...
    for perm in valid_perms:
        ...
        try:
            ...
            # if doctype not found (may be due to reid) it should not be considered for skip
            continue
