frappe.ui.form.ControlCode = class ControlCode extends frappe.ui.form.ControlText {
    ...
    make_ace_editor() {
        ...
        if (this.df.max_lines || this.df.min_lines || this.df.max_height) {
            ...
            this.expanded = false;
            this.$expand_button = $(`<button class="btn btn-xs btn-default mt-2">${this.get_button_label()}</button>`)
                .click(() => {
    ...
    set_language() {
        ...
        this.editor.setKeyboardHandler(`ace/keyboard/${frappe.boot.user.code_editor_type || "vscode"}`);
    }
