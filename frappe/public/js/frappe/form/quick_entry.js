...
frappe.quick_edit = function (doctype, id) {
    if (!id) id = doctype; // single
    frappe.db.get_doc(doctype, id).then((doc) => {
...
frappe.ui.form.QuickEntryForm = class QuickEntryForm extends frappe.ui.Dialog {
        ...
        return new Promise((resolve) => {
            frappe.model.with_doctype(this.doctype, () => {
                ...
                if (this.is_quick_entry() || this.force) {
                    ...
                    frappe.set_route("Form", this.doctype, this.doc.id).then(() => resolve(this));
    ...
    set_meta_and_mandatory_fields() {
        ...
        this.docfields = fields.filter((df) => {
            return (
                (df.reqd || df.allow_in_quick_entry) && !df.read_only && !df.is_virtual && df.fieldtype !== "Tab Break"
            );
        });
    }
        ...
        this.set_primary_action(__("Save"), function () {
            ...
            if (data) {
                ...
                me.script_manager.trigger("validate").then(() => {
                    me.insert().then(() => {
                        let messagetxt = __("{1} saved", [__(me.doctype), `<b>${this.doc.id}</b>`]);
    ...
    open_doc(set_hooks) {
        ...
        frappe.set_route("Form", this.doctype, this.doc.id);
    }
