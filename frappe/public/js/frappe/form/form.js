...
frappe.ui.form.Form = class FrappeForm {
    constructor(doctype, parent, in_form, doctype_layout_id) {
        this.docid = "";
        this.doctype = doctype;
        this.doctype_layout_id = doctype_layout_id;
    ...
    watch_model_updates() {
		...
		$.each(table_fields, function (i, df) {
			frappe.model.on(
				...
				"*",
				function (fieldname, value, doc, skip_dirty_trigger = false) {
					if (doc.parent == me.id && doc.parentfield === df.fieldname) {
						if (!skip_dirty_trigger) {
							me.dirty();
						}
	...
	setup_notify_on_rename() {
      $(document).on("reid", (ev, dt, old_id, new_id) => {
			if (dt == this.doctype) this.reid_notify(dt, old_id, new_id);
		});

		frappe.realtime.on("doc_reid", (data) => {
			// the current form has been reided by some backend process
    ...
    trigger_onload(switched) {
        if (!this.opendocs[this.docid]) {
            ...
            if (this.doc.localid) {
                // trigger form-reid and remove .localid
                delete this.doc.localid;
                $(document).trigger("form-reid", [this]);
            }
        }
    }
    ...
    initialize_new_doc() {
        ...
        if (this.meta.track_seen) {
            $('.list-id[data-name="' + me.docid + '"]').addClass("seen");
        }
    }
    ...
    _cancel(btn, callback, on_error, skip_confirm) {
        ...
        if (skip_confirm) {
            ...
            frappe.confirm(
                __("Permanently Cancel {0}?", [this.docid]),
                cancel_doc,
                me.handle_save_fail(btn, on_error)
            );
        }
    ...
    // reid the form
    // notify this form of reidd records
    reid_notify(dt, old, id) {
        // from form
    ...
    reid_doc() {
        frappe.model.reid_doc(this.doctype, this.docid, () => this.refresh_header());
    }
    ...
    update_in_all_rows(table_fieldname, fieldname, value) {
        ...
        frappe.model
            ...
            .forEach((child) =>
                frappe.model.set_value(child.doctype, child.id, fieldname, value)
            );
    }
