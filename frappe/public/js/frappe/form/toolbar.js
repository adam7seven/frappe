...
frappe.ui.form.Toolbar = class Toolbar {
    ...
    set_title() {
        ...
        if (this.frm.meta.issingle && this.frm.meta.is_virtual) {
            ...
            if (this.frm.doc.__islocal || title === this.frm.docid || this.frm.meta.autoname === "hash") {
    ...
    rename_document_title(input_id, input_title, merge = false) {
            ...
            return frappe
                ...
                .then((new_docid) => {
                    const reload_form = (input_id) => {
                        $(document).trigger("rename", [doctype, docid, input_id]);
                    ...
                    if (input_id != docid) {
                        frappe.realtime.on("list_update", (data) => {
                            if (data.doctype == doctype && data.id == input_id) {
                                ...
                                frappe.show_alert({
                                    message: __("Document renamed from {0} to {1}", [docid.bold(), input_id.bold()]),
                                    indicator: "success",
                                });
                            }
                        });
        ...
        this.page.$title_area.find(".title-text").on("click", () => {
            ...
            if (fields.length > 0) {
                ...
                d.set_primary_action(__("Rename"), (values) => {
                    ...
                    this.rename_document_title(values.id, values.title, values.merge)
                        .then(() => {
                            d.hide();
                        })
                        .catch(() => {
