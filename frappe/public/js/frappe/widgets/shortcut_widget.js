...
export default class ShortcutWidget extends Widget {
    ...
    set_actions() {
        ...
        $(frappe.utils.icon("es-line-arrow-up-right", "xs", "", "", "ml-2")).appendTo(this.action_area);
        ...
        if (this.type == "DocType" && this.doc_view != "New" && !frappe.boot.single_types.includes(this.link_to)) {
            frappe.db
                .count(this.link_to, {
                    filters: filters || []
                })
    ...
    set_count(count) {
        ...
        $(`<div class="indicator-pill no-indicator-dot ellipsis ${color}">${__(label)}</div>`).appendTo(
            this.action_area
        );
