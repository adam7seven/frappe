            ...
            fields: [
                {
                    ...
                    label: __("Search"),
                    reqd: 1,
                    get_query: "frappe.integrations.doctype.geolocation_settings.geolocation_settings.autocomplete",
                    onchange: () => {
                        ...
                        frappe.model.with_doctype("Address", () => {
                            ...
                            const mandatory_fields = frappe
                                .get_meta("Address")
                                .fields.filter(
                                    field => field.reqd && !field.default && field.fieldname !== "address_type"
                                );
                            const missing_fields = mandatory_fields.filter(field => !address[field.fieldname]);
