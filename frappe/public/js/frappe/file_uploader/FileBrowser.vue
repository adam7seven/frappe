	...
	<div class="file-browser">
		...
		<div class="file-browser-list">
			<div class="file-filter">
				<input type="search" class="form-control input-xs" :placeholder="__('Search by filename or extension')"
					v-model="search_text" @input="frappe.utils.debounce(search_by_id(), 300)" />
			</div>
			<TreeNode class="tree with-skeleton" :node="node" :selected_node="selected_node"
				@node-click="(n) => toggle_node(n)" @load-more="(n) => load_more(n)" />
		</div>
	</div>
</template>
...
<script setup>
	...
	function make_file_node(file) {
		let filename = file.file_name || file.id;
