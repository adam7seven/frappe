...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
    ...
    export_report() {
        ...
        this.export_dialog = frappe.report_utils.get_export_dialog(
            __(this.report_name),
            extra_fields,
            ({ file_format, include_indentation, include_filters, csv_delimiter, csv_quoting, csv_decimal_sep }) => {
                        ...
                        fields: [
                            {
                                ...
                                change: () => {
                                    const { doctype, fieldname } = JSON.parse(d.get_value("doctype"));
                        ...
                        primary_action: values => {
                            ...
                            frappe.call({
                                ...
                                args: {
                                    field: values.field,
                                    doctype: values.doctype,
                                    names: Array.from(this.doctype_field_map[values.doctype][fieldname])
                                },
                                callback: r => {
    ...
    get_linked_doctypes() {
        ...
        this.data.forEach(row => {
            doctypes.forEach(doc => {
                row[doc.fieldname] && this.doctype_field_map[doc.doctype][doc.fieldname].add(row[doc.fieldname]);
            });
        });
