...
frappe.views.ImageView = class ImageView extends frappe.views.ListView {
	...
	item_html(item) {
		...
		const _html = item._image_url
			? `<img data-name="${encoded_id}" src="${item._image_url}" alt="${title}">`
		...
		const expand_button_html = item._image_url
			? `<div class="zoom-view" data-name="${encoded_id}">
					...
					<div class="image-title">
						<span class="ellipsis" title="${escaped_title}">
							<a class="ellipsis" href="${this.get_form_link(item)}"
								title="${escaped_title}" data-doctype="${this.doctype}" data-name="${item.id}">
								${title}
							</a>
						</span>
					</div>
...
frappe.views.GalleryView = class GalleryView {
	...
	prepare_pswp_items(_items, _images_map) {
		...
		return new Promise((resolve) => {
			const items = this.items.map(function (i) {
				const query = 'img[data-name="' + i._id + '"]';
				...
				if (!el) {
					el = me.wrapper.find('.image-field[data-name="' + i._id + '"]').get(0);
	...
	show(docid) {
		this.lib_ready.then(() => this.prepare_pswp_items()).then(() => this._show(docid));
	}
	_show(docid) {
		const me = this;
		...
		var options = {
			...
			getThumbBoundsFn: function (index) {
				const query = 'img[data-name="' + items[index]._id + '"]';
	...
	browse_images() {
		...
		this.pswp.listen("afterChange", function () {
			const images = images_map[this.currItem.id];
		...
		$more_items.on("click", ".pswp__more-item", (e) => {
			...
			const index = this.pswp.items.findIndex((i) => i.id === this.pswp.currItem.id);
