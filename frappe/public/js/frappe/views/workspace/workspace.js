...
frappe.views.Workspace = class Workspace {
    ...
    get_page_to_show() {
        ...
        if (frappe.boot.user.default_workspace) {
            ...
            this.sidebar.all_pages.filter(page => page.name == localStorage.current_page).length != 0
        ) {
            default_page = {
    ...
    async show_page(page) {
        ...
        if (this.sidebar.all_pages.length) {
            ...
            this.wrapper.find(".workspace-icon").html(frappe.utils.icon(this._page.icon || "folder-normal", "md"));
        ...
        return this.editor
            ...
            .then(outputData => {
                ...
                let blocks = outputData.blocks.filter(
                    item =>
                        item.type != "card" ||
                        (item.data.card_name !== "Custom Documents" && item.data.card_name !== "Custom Reports")
                );
