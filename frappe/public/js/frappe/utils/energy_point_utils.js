...
Object.assign(frappe.energy_points, {
	...
	get_history_log_message(log) {
		const owner_id = frappe.user.full_name(log.owner).bold();
		const ref_doc = log.reference_id;

		if (log.type === "Appreciation") {
			return __("{0} appreciated on {1}", [owner_id, ref_doc]);
		}
		if (log.type === "Criticism") {
			return __("{0} criticized on {1}", [owner_id, ref_doc]);
		}
		if (log.type === "Revert") {
	...
	get_form_log_message(log) {
		...
		if (log.type === "Appreciation") {
			return __("{0} appreciated {1}", [owner_id, user]);
		}
		if (log.type === "Criticism") {
			return __("{0} criticized {1}", [owner_id, user]);
		}
		if (log.type === "Revert") {
			return __("{0} reverted {1}", [owner_id, log.revert_of]);
		}
