<template>
	<div class="form-section-container" :style="{ borderBottom: props.section.df.hide_border ? 'none' : '' }">
		<div :class="[
			'form-section',
			hovered ? 'hovered' : '',
			store.selected(section.df.id) ? 'selected' : '',
		]" :title="section.df.fieldname" @click.stop="select_section" @mouseover.stop="hovered = true"
			...
			<div :class="[
				...
				<div class="section-label">
					<EditableInput :text="section.df.label" :placeholder="__('Section Title')"
						v-model="section.df.label" />
					<div v-if="section.df.collapsible" class="collapse-indicator"
						v-html="frappe.utils.icon(collapsed ? 'down' : 'up-line', 'sm')"></div>
				</div>
			...
			<div class="section-columns" :class="{
				hidden: section.df.collapsible && collapsed,
				'has-one-column': section.columns.length === 1,
			}">
				<draggable class="section-columns-container" v-model="section.columns" group="columns" item-key="id"
					:delay="is_touch_screen_device() ? 200 : 0" :animation="200" :easing="store.get_animation"
					:disabled="store.read_only">
					<template #item="{ element }">
						<Column :section="section" :column="element"
...
<script setup>
	...
	const selected = computed(() => store.selected(props.section.df.id));
