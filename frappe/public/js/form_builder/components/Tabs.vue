<script setup>
	...
	const selected = computed(() => store.selected(store.current_tab.df.id));
	const has_tabs = computed(() => store.form.layout.tabs.length > 1);
	store.form.active_tab = store.form.layout.tabs[0].df.id;
	...
	function drag_over(tab) {
		!dragged.value &&
			setTimeout(() => {
				store.form.active_tab = tab.df.id;
			}, 500);
	}
	...
	function delete_tab(tab, with_children) {
		...
		store.form.active_tab = tabs[prev_tab_index].df.id;
...
<template>
	<div class="tab-header" v-if="store.form.layout.tabs.length > 1">
		<draggable v-show="has_tabs" class="tabs" v-model="store.form.layout.tabs" group="tabs"
			:delay="is_touch_screen_device() ? 200 : 0" :animation="200" :easing="store.get_animation" item-key="id"
			:disabled="store.read_only">
			<template #item="{ element }">
				<div :class="['tab', store.form.active_tab == element.df.id ? 'active' : '']"
					:title="element.df.fieldname" :data-is-user-generated="store.is_user_generated_field(element)"
					@click.stop="activate_tab(element)" @dragstart="dragged = true" @dragend="dragged = false"
					@dragover="drag_over(element)">
					<EditableInput :text="element.df.label" :placeholder="__('Tab Label')" v-model="element.df.label" />
					<button class="remove-tab-btn btn btn-xs" :title="__('Remove tab')"
						@click.stop="remove_tab(element, $event)" :hidden="store.read_only">
		...
		<div class="tab-actions" :hidden="store.read_only">
			<button class="new-tab-btn btn btn-xs" :class="{ 'no-tabs': !has_tabs }" :title="__('Add new tab')"
				@click="add_new_tab">
	...
	<div class="tab-contents">
		<div class="tab-content" v-for="(tab, i) in store.form.layout.tabs" :key="i"
			:class="[store.form.active_tab == tab.df.id ? 'active' : '']">
			<draggable class="tab-content-container" v-model="tab.sections" group="sections"
				:delay="is_touch_screen_device() ? 200 : 0" :animation="200" :easing="store.get_animation" item-key="id"
				:disabled="store.read_only">
				<template #item="{ element }">
