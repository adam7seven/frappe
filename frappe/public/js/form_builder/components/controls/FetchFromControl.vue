...
<script setup>
	...
	let field_df = computedAsync(async () => {
		...
		let doctype_id = doctypes.value?.find((df) => df.value == doctype.value).doctype_id;
		if (!doctype_id) return df;

		if (props.value.split(".")[0] != doctype.value) {
			fieldname.value = "";
		}

		await load_doctype_model(doctype_id);

		let fields = frappe.meta
			.get_docfields(doctype_id, null, {
	...
	watch([() => doctype.value, () => fieldname.value], ([doctype_value, fieldname_value]) => {
		...
		if (doctype_value != doctype_id || fieldname_value != field_name) {
...
<template>
	<div>
		...
		<SelectControl v-if="doctype" :df="field_df" :read_only="read_only" :value="fieldname" v-model="fieldname"
			:no_label="true" />
	</div>
</template>
