<script setup>
    ...
    function get_option(opt) {
        ...
        if (comma_index === -1) {
            //opt not has comma, the opt is value
            return { label: __(opt), value: opt };
        }
        else if (comma_index === 0) {
            //the first char is comma, value is empty string, and label is chars after comma.
            return { label: __(opt.substring(1)), value: "" };
        }
        //split by first comma, the first part is value, and the last a
        return { label: __(opt.substring(comma_index + 1)), value: opt.substring(0, comma_index) };
    }
