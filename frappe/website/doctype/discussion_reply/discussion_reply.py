...
class DiscussionReply(Document):
    ...
    def after_delete(self):
        frappe.publish_realtime(
            ...
            room=get_website_room(),
            message={"reply_id": self.id},
            after_commit=True,
        )


@frappe.whitelist()
def delete_message(reply_id):
    owner = frappe.db.get_value("Discussion Reply", reply_id, "owner")
    if owner == frappe.session.user:
        frappe.delete_doc("Discussion Reply", reply_id)
