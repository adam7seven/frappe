...
class HelpArticle(WebsiteGenerator):
    ...
    def set_route(self):
        ...
        if not self.route:
            self.route = "/".join(
                [
                    frappe.get_value("Help Category", self.category, "route"),
    ...
    def update_category(self):
        cnt = frappe.db.count(
            "Help Article", filters={"category": self.category, "published": 1}
        )
    ...
    def get_parents(self, context):
        return [
            {"title": context.category.category_id, "route": context.category.route}
        ]
...
def add_feedback(article: str, helpful: str):
    ...
    frappe.db.set_value(
        "Help Article", article, field, value + 1, update_modified=False
    )
