...
class WebsiteTheme(Document):
    ...
    def generate_bootstrap_theme(self):
        ...
        folder_path = abspath(
            frappe.utils.get_files_path("website_theme", is_private=False)
        )
        ...
        process = Popen(
            command, cwd=frappe.get_app_source_path("frappe"), stdout=PIPE, stderr=PIPE
        )
    ...
    def delete_old_theme_files(self, folder_path):
        ...
        for fname in os.listdir(folder_path):
            if fname.startswith(frappe.scrub(self.name) + "_") and fname.endswith(
                ".css"
            ):
