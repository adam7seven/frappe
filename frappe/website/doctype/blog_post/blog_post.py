...
def get_list_context(context=None):
    ...
    category_id = frappe.utils.escape_html(frappe.local.form_dict.blog_category or frappe.local.form_dict.category)
    if category_id:
...
def get_blog_categories():
    ...
    return (
        ...
        .where(
            ...
            & ExistsCriterion(
                frappe.qb.from_(post).select("id").where((post.published == 1) & (post.blog_category == category.id))
            )
        )
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
    ...
    for post in posts:
        ...
        post.category = frappe.db.get_value("Blog Category", post.blog_category, ["id", "route", "title"], as_dict=True)
