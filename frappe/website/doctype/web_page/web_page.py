    ...
    def get_context(self, context):
        ...
        if self.context_script:
            ...
            safe_exec(
                ...
                None,
                _locals,
                script_filename=f"web page {self.id}",
            )
    ...
    def check_for_redirect(self, context):
        if "<!-- redirect:" in context.main_section:
            frappe.local.flags.redirect_location = (
                context.main_section.split("<!-- redirect:", 2)[1]
                .split("-->", 1)[0]
                .strip()
            )
...
def check_publish_status():
    ...
    web_pages = frappe.get_all(
        "Web Page", fields=["id", "published", "start_date", "end_date"]
    )
...
def get_dynamic_web_pages() -> dict[str, str]:
    ...
    get_web_pages_with_dynamic_routes = (
        frappe.get_hooks("get_web_pages_with_dynamic_routes") or []
    )
