...
def process_data_deletion_request():
    ...
    requests = frappe.get_all("Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="id")
    ...
    for request in requests:
        ...
        if time_diff_in_hours(get_datetime(), doc.creation) >= auto_account_deletion:
            doc.add_comment(
                "Comment",
                _("The User record for this request has been auto-deleted due to inactivity by system admins."),
            )
