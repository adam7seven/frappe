        ...
        for basepath, folders, files in os.walk(start_path):  # noqa: B007
            for fname in files:
                ...
                if extn in ("html", "xml", "js", "css", "md"):
                    page_info = get_page_info(os.path.join(basepath, fname), app, start, basepath, app_path, fname)
...
def get_page_info(path, app, start, basepath=None, app_path=None, fname=None):
    ...
    if os.path.exists(page_info.controller_path):
        controller = app + "." + os.path.relpath(page_info.controller_path, app_path).replace(os.path.sep, ".")[:-3]
...
def get_doctypes_with_web_view():
    ...
    def _get():
        ...
        doctypes_with_web_view = frappe.get_all("DocType", fields=["id", "module"], filters=dict(has_web_view=1))
        module_app_map = frappe.local.module_app
        doctypes += [
            d.id for d in doctypes_with_web_view if module_app_map.get(frappe.scrub(d.module)) in installed_apps
        ]
        return doctypes
