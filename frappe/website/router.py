        ...
        for basepath, folders, files in os.walk(start_path):  # noqa: B007
            for fname in files:
                ...
                if extn in ("js", "css") and os.path.exists(
                    os.path.join(basepath, page_id + ".html")
                ):
...
def get_page_info(path, app, start, basepath=None, app_path=None, fname=None):
    ...
    if os.path.exists(page_info.controller_path):
        controller = (
            app
            + "."
            + os.path.relpath(page_info.controller_path, app_path).replace(
                os.path.sep, "."
            )[:-3]
        )
...
def setup_source(page_info):
    ...
    css_path = os.path.join(
        page_info.basepath, (page_info.basename or "index") + ".css"
    )
