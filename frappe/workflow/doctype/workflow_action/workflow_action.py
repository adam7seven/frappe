...
def on_doctype_update():
    ...
    frappe.db.add_index(
        "Workflow Action", ["reference_id", "reference_doctype", "status"]
    )
...
def return_link_expired_page(doc, doc_workflow_state):
    frappe.respond_as_web_page(
        ...
        _("Document {0} has been set to state {1} by {2}").format(
            ...
            frappe.bold(frappe.get_value("User", doc.get("modified_by"), "full_id")),
        ),
        indicator_color="blue",
    )
...
def get_next_possible_transitions(workflow_id, state, doc=None):
    ...
    for transition in transitions:
        is_next_state_optional = get_state_optional_field_value(
            workflow_id, transition.next_state
        )
...
def deduplicate_actions(action_list):
    ...
    for action_data in action_list:
        if not action_map.get(action_data.action_id):
            action_map[action_data.action_id] = action_data
...
def get_email_template_from_workflow(doc):
    ...
    template_id = frappe.db.get_value(
...
def get_state_optional_field_value(workflow_id, state):
    return frappe.get_cached_value(
        "Workflow Document State",
        {"parent": workflow_id, "state": state},
        "is_optional_state",
    )
