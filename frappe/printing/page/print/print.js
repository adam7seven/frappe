...
frappe.ui.form.PrintView = class {
        ...
        if (!cint(this.print_settings.repeat_header_footer)) {
            description =
                ...
                __("Footer might not be visible as {0} option is disabled</div>", [
                    `<a href="/app/print-settings/Print Settings">${__("Repeat Header and Footer")}</a>`
                ]);
        }
    ...
    set_default_print_format() {
        if (frappe.meta.get_print_formats(this.frm.doctype).includes(this.print_format_selector.val())) return;
