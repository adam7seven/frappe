...
def get_decrypted_password(doctype, id, fieldname="password", raise_exception=True):
    result = (
        ...
        .where((Auth.doctype == doctype) & (Auth.id == id) & (Auth.fieldname == fieldname) & (Auth.encrypted == 1))
        .limit(1)
    ).run()
...
def set_encrypted_password(doctype, id, pwd, fieldname="password"):
    query = (
        frappe.qb.into(Auth)
        .columns(Auth.doctype, Auth.id, Auth.fieldname, Auth.password, Auth.encrypted)
        .insert(doctype, id, fieldname, encrypt(pwd), 1)
    )
    ...
    if frappe.db.db_type == "mariadb":
        ...
        query = query.on_conflict(Auth.doctype, Auth.id, Auth.fieldname).do_update(Auth.password)
...
def check_password(user, pwd, doctype="User", fieldname="password", delete_tracker_cache=True):
    ...
    result = (
        ...
        .where((Auth.doctype == doctype) & (Auth.id == user) & (Auth.fieldname == fieldname) & (Auth.encrypted == 0))
        .limit(1)
        .run(as_dict=True)
    )
...
def update_password(user, pwd, doctype="User", fieldname="password", logout_all_sessions=False):
    ...
    # TODO: Simplify this via aliasing methods in `frappe.qb`
    if frappe.db.db_type == "mariadb":
        query = query.on_duplicate_key_update(Auth.password, hashPwd).on_duplicate_key_update(Auth.encrypted, 0)
    elif frappe.db.db_type == "postgres":
        query = (
            query.on_conflict(Auth.doctype, Auth.id, Auth.fieldname)
