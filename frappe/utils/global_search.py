...
def sync_values(values: list):
    ...
    query = (
        frappe.qb.into(GlobalSearch)
        .columns(["doctype", "id", *conflict_fields])
        .insert(*values)
    )
...
def search(text, start=0, limit=20, doctype=""):
    ...
    allowed_doctypes = set(get_doctypes_for_global_search()) & set(
    ...
    for word in set(text.split("&")):
        ...
        query = (
            ...
            .select(
                global_search.doctype, global_search.id, global_search.content, rank
            )
