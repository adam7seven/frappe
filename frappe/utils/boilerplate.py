...
def _create_app_boilerplate(dest, hooks, no_git=False):
    ...
    with open(
        os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w"
    ) as f:
...
class PatchCreator:
    ...
    def fetch_user_inputs(self):
        self._ask_app_name()
        self._ask_doctype_id()
        self._ask_patch_meta_info()

    def _ask_app_name(self):
        self.app = click.prompt(
            "Select app for new patch", type=click.Choice(self.all_apps)
        )
        self.app_dir = pathlib.Path(frappe.get_app_path(self.app))

    def _ask_doctype_id(self):
        def _doctype_id(filename):
            with contextlib.suppress(Exception):
                with open(filename) as f:
                    return json.load(f).get("id")

        doctype_files = list(glob.glob(f"{self.app_dir}/**/doctype/**/*.json"))
        doctype_map = {_doctype_id(file): file for file in doctype_files}
        doctype_map.pop(None, None)

        doctype = click.prompt(
            "Provide DocType id on which this patch will apply",
