...
def pdf_body_html(template, args, **kwargs):
    try:
        ...
        # Guess line number ?
        frappe.throw(
            _("Error in print format on line {0}: {1}").format(_guess_template_error_line_number(template), e),
...
def prepare_options(html, options):
    ...
    # page size
    pdf_page_size = options.get("page-size") or frappe.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
