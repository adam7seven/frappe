...
class BackupGenerator:
    ...
    def get_backup(self, older_than=24, ignore_files=False, force=False):
        ...
        if not (
            self.backup_path_conf and self.backup_path_db and self.backup_path_files and self.backup_path_private_files
        ):
        ...
        if not (last_db and last_file and last_private_file and site_config_backup_path):
            ...
            if not ignore_files:
                self.delete_if_step_fails(self.backup_files, self.backup_path_files, self.backup_path_private_files)
    ...
    def take_dump(self):
        ...
        if not gzip_exc:
            frappe.throw(_("gzip not found in PATH! This is required to take a backup."), exc=frappe.ExecutableNotFound)
