...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
    ...
    def time_cache_wrapper(func: Callable | None = None) -> Callable:
        ...
        if ttl is not None and not callable(ttl):
            func.ttl = ttl
            func.expiration = datetime.datetime.now(pytz.UTC) + datetime.timedelta(
                seconds=func.ttl
            )
        ...
        def site_cache_wrapper(*args, **kwargs):
            if getattr(frappe.local, "initialised", None):
                ...
                if (
                    hasattr(func, "ttl")
                    and datetime.datetime.now(pytz.UTC) >= func.expiration
                ):
                    func.clear_cache()
                    func.expiration = datetime.datetime.now(
                        pytz.UTC
                    ) + datetime.timedelta(seconds=func.ttl)

                if (
                    hasattr(func, "maxsize")
                    and len(_SITE_CACHE[func_key][frappe.local.site]) >= func.maxsize
                ):
                ...
                if func_call_key not in _SITE_CACHE[func_key][frappe.local.site]:
                    _SITE_CACHE[func_key][frappe.local.site][func_call_key] = func(
                        *args, **kwargs
                    )
