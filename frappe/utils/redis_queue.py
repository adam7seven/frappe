...
class RedisQueue:
    ...
    def get_connection(cls, username=None, password=None):
        if frappe.conf.redis_queue_sentinel_enabled:
            ...
            sentinels = [
                tuple(node.split(":"))
                for node in frappe.conf.get("redis_queue_sentinels", [])
            ]
            sentinel = get_sentinel_connection(
                ...
                master_username=frappe.conf.get(
                    "redis_queue_master_username", username
                ),
                master_password=frappe.conf.get(
                    "redis_queue_master_password", password
                ),
            )
            ...
            return conn
        conn = redis.from_url(
            frappe.conf.redis_queue, username=username, password=password
        )
        conn.ping()
        return conn
