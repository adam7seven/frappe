...
def safe_eval(code, eval_globals=None, eval_locals=None):
    ...
    return eval(
        compile_restricted(
            code, filename="<safe_eval>", policy=FrappeTransformer, mode="eval"
        ),
        eval_globals,
        eval_locals,
    )
...
def is_job_queued(job_id, queue="default"):
    ...
    return queued_jobs and job_id in queued_jobs


def safe_enqueue(function, **kwargs):
    """
    Enqueue function to be executed using a background worker
    Accepts frappe.enqueue params like job_id, queue, timeout, etc.
...
def _get_attr_for_eval(object, name, default=ARGUMENT_NOT_SET):
    ...
    return (
        getattr(object, name)
        if default is ARGUMENT_NOT_SET
        else getattr(object, name, default)
    )
