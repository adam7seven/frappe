...
def add_random_children(
    doc: "Document", fieldname: str, rows, randomize: dict, unique=None
):
    nrows = rows
    if rows > 1:
...
def can_make(doctype: str) -> bool:
    return (
        random.random() < settings.prob.get(doctype, settings.prob["default"])["make"]
    )


def how_many(doctype: str) -> int:
    return random.randrange(
        *settings.prob.get(doctype, settings.prob["default"])["qty"]
    )
