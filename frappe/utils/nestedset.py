...
class NestedSet(Document):
    ...
    def validate_if_child_exists(self):
        ...
        if has_children:
            frappe.throw(_("Cannot delete {0} as it has child nodes").format(self.id), NestedSetChildExistsError)
    ...
    def validate_ledger(self, group_identifier="is_group"):
        if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
            if frappe.get_all(self.doctype, {self.nsm_parent_field: self.id, "docstatus": ("!=", 2)}):
                frappe.throw(_("{0} {1} cannot be a leaf node as it has children").format(_(self.doctype), self.id))
