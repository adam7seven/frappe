...
def update_nsm(doc):
    ...
    parent, old_parent = (
        doc.get(parent_field) or None,
        doc.get(old_parent_field) or None,
    )
    ...
    frappe.db.set_value(
        doc.doctype, doc.id, old_parent_field, parent or "", update_modified=False
    )
...
def update_add_node(doc, parent, parent_field):
    ...
    if parent:
        left, right = frappe.db.get_value(
            doctype, {"id": parent}, ["lft", "rgt"], for_update=True
        )
    ...
    frappe.qb.update(Table).set(Table.rgt, Table.rgt + 2).where(
        Table.rgt >= right
    ).run()
    frappe.qb.update(Table).set(Table.lft, Table.lft + 2).where(
        Table.lft >= right
    ).run()
...
def rebuild_tree(doctype: str) -> None:
    ...
    if not meta.has_field("lft") or not meta.has_field("rgt"):
        frappe.throw(
            _("Rebuilding of tree is not supported for {}").format(
                frappe.bold(doctype)
            ),
...
def rebuild_node(doctype, parent, left, parent_field):
    ...
    frappe.db.set_value(
        doctype, parent, {"lft": left, "rgt": right}, update_modified=False
    )
...
def validate_loop(doctype, id, lft, rgt):
    ...
    if id in frappe.get_all(
        doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="id"
    ):
        frappe.throw(
            _("Item cannot be added to its own descendants"), NestedSetRecursionError
        )
...
def remove_subtree(doctype: str, id: str, throw=True):
    ...
    frappe.qb.update(table).set(table.lft, table.lft - width).where(
        table.lft > rgt
    ).run()
    frappe.qb.update(table).set(table.rgt, table.rgt - width).where(
        table.rgt > rgt
    ).run()
...
class NestedSet(Document):
    ...
    def validate_if_child_exists(self):
        has_children = frappe.db.count(
            self.doctype, filters={self.nsm_parent_field: self.id}
        )
        if has_children:
            frappe.throw(
                _("Cannot delete {0} as it has child nodes").format(self.id),
                NestedSetChildExistsError,
            )

    def before_reid(self, olddn, newdn, merge=False, group_fname="is_group"):
    ...
    def after_reid(self, olddn, newdn, merge=False):
    ...
    def validate_ledger(self, group_identifier="is_group"):
        if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
            if frappe.get_all(
                self.doctype, {self.nsm_parent_field: self.id, "docstatus": ("!=", 2)}
            ):
                frappe.throw(
                    _("{0} {1} cannot be a leaf node as it has children").format(
                        _(self.doctype), self.id
                    )
                )
    ...
    def get_children(self) -> Iterator["NestedSet"]:
        ...
        child_ids = frappe.get_list(
            self.doctype, filters={self.nsm_parent_field: self.id}, pluck="id"
        )
        for id in child_ids:
...
def get_root_of(doctype):
    ...
    node_query = SubQuery(
        frappe.qb.from_(t2)
        .select(Count("*"))
        .where((t2.lft < t1.lft) & (t2.rgt > t1.rgt))
    )
    result = (
        frappe.qb.from_(t1)
        .select(t1.id)
        .where((node_query == 0) & (t1.rgt > t1.lft))
        .run()
    )
...
def get_descendants_of(
    doctype, id, order_by="lft desc", limit=None, ignore_permissions=False
):
