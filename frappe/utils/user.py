...
class UserPermissions:
    ...
    def build_permissions(self):
        ...
        self.shared = frappe.get_all("DocShare", {"user": self.id, "read": 1}, distinct=True, pluck="share_doctype")
...
def get_users_with_role(role: str) -> list[str]:
    ...
    return (
        ...
        .from_(User)
        .where(
            (HasRole.role == role) & (User.id != "Administrator") & (User.enabled == 1) & (HasRole.parent == User.id)
        )
        .select(User.id)
        .distinct()
        .run(pluck=True)
