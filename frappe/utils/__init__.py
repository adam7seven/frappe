...
def get_fullname(user=None):
    ...
    if not frappe.local.fullnames.get(user):
        ...
        if p:
            frappe.local.fullnames[user] = " ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
        else:
...
def execute_in_shell(cmd, verbose=False, low_priority=False, check_exit_code=False):
    ...
    if failed:
        raise frappe.CommandFailedError("Command failed", out.decode(errors="replace"), err.decode(errors="replace"))
...
def get_sites(sites_path=None):
    ...
    for site in os.listdir(sites_path):
        ...
        if os.path.isdir(path) and not os.path.islink(path) and os.path.exists(os.path.join(path, "site_config.json")):
