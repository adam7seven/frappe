...
def _get_traceback_sanitizer():
    ...
    return Format(
        custom_var_printers=[
            # redact variables
            *[
                (variable_name, lambda *a, **kw: placeholder)
                for variable_name in blocklist
            ],
...
def get_bench_id():
    return frappe.get_conf().get(
        "bench_id", get_bench_path().strip("/").replace("/", "-")
    )
...
def check_format(email_id):
    ...
    try:
        pos = email_id.rindex("@")
        is_valid = (
            pos > 0 and (email_id.rindex(".") > pos) and (len(email_id) - pos > 4)
        )
    except Exception:
        # print(e)
        pass
...
def get_installed_apps_info():
    ...
    out.extend(
        {
            "app_name": app,
            "version": version_details.get("branch_version")
            or version_details.get("version"),
...
def get_build_version():
    try:
        return str(
            os.path.getmtime(
                os.path.join(frappe.local.sites_path, "assets/assets.json")
            )
        )
    except OSError:
...
def add_user_info(
    user: str | list[str] | set[str], user_info: dict[str, _UserInfo]
) -> None:
    if not user:
        return
