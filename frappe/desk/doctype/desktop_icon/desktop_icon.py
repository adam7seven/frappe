...
def after_doctype_insert():
    frappe.db.add_unique("Desktop Icon", ("module_id", "owner", "standard"))
    ...
    if not user_icons:
        ...
        for icon in standard_icons:
            if icon._doctype in blocked_doctypes:
                icon.blocked = 1
            standard_map[icon.module_id] = icon
        ...
        for icon in user_icons:
            standard_icon = standard_map.get(icon.module_id, None)
        ...
        user_icon_ids = [icon.module_id for icon in user_icons]
        for standard_icon in standard_icons:
            if standard_icon.module_id not in user_icon_ids:
        ...
        for icon in user_icons:
            if icon.module_id in user_blocked_modules:
                icon.hidden = 1
...
def add_user_icon(
    ...
    icon_id = frappe.db.exists(
        "Desktop Icon",
        {"standard": standard, "link": link, "owner": frappe.session.user},
    )

    if icon_id:
        if frappe.db.get_value("Desktop Icon", icon_id, "hidden"):
            # if it is hidden, unhide it
            frappe.db.set_value("Desktop Icon", icon_id, "hidden", 0)
            clear_desktop_icons_cache()

    else:
        idx = (
            frappe.db.sql(
                "select max(idx) from `tabDesktop Icon` where owner=%s",
                frappe.session.user,
            )[0][0]
            or frappe.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[
                0
            ][0]
        )
        ...
        module_icon = frappe.get_value(
            "Desktop Icon",
            {"standard": 1, "module_id": userdefined_icon.module},
...
def set_order(new_order, user=None):
    ...
    for i, module_id in enumerate(new_order):
        if module_id not in ("Explore",):
            if user:
                icon = get_user_copy(module_id, user)
            else:
                id = frappe.db.get_value(
                    "Desktop Icon", {"standard": 1, "module_id": module_id}
                )
                if id:
                    icon = frappe.get_doc("Desktop Icon", id)
                else:
                    # standard icon missing, create one for DocType
                    id = add_user_icon(module_id, standard=1)
                    icon = frappe.get_doc("Desktop Icon", id)
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
    ...
    if not frappe.flags.keep_desktop_icons:
        frappe.db.sql(
            "update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1"
        )

    # set as visible if present, or add icon
    for module_id in list(visible_list):
        id = frappe.db.get_value("Desktop Icon", {"module_id": module_id})
        if id:
            frappe.db.set_value("Desktop Icon", id, "hidden", 0)
        else:
            if frappe.db.exists("DocType", module_id):
                try:
                    add_user_icon(module_id, standard=1)
                except frappe.UniqueValidationError as e:
                    if not ignore_duplicate:
                        raise e
                    else:
                        visible_list.remove(module_id)
...
def set_hidden_list(hidden_list, user=None):
    ...
    # set as hidden
    for module_id in hidden_list:
        set_hidden(module_id, user, 1)

    # set as seen
    for module_id in list(set(get_all_icons()) - set(hidden_list)):
        set_hidden(module_id, user, 0)
...
def set_hidden(module_id, user=None, hidden=1):
    ...
    if user:
        ...
        icon = frappe.get_doc("Desktop Icon", {"standard": 1, "module_id": module_id})
...
def get_all_icons():
    return [
        ...
        for d in frappe.get_all(
            "Desktop Icon", filters={"standard": 1}, fields=["module_id"]
        )
    ]
...
def get_user_copy(module_id, user=None):
    """Return user copy (Desktop Icon) of the given module_id. If user copy does not exist, create one.
    ...
    desktop_icon_id = frappe.db.get_value(
        "Desktop Icon", {"module_id": module_id, "owner": user, "standard": 0}
    )

    if desktop_icon_id:
        return frappe.get_doc("Desktop Icon", desktop_icon_id)
    else:
        return make_user_copy(module_id, user)


def make_user_copy(module_id, user):
    """Insert and return the user copy of a standard Desktop Icon"""
    standard_name = frappe.db.get_value(
        "Desktop Icon", {"module_id": module_id, "standard": 1}
    )

    if not standard_name:
        frappe.throw(_("{0} not found").format(module_id), frappe.DoesNotExistError)
...
def sync_from_app(app):
    ...
    for i, m in enumerate(modules_list):
        desktop_icon_id = frappe.db.get_value(
            "Desktop Icon", {"module_id": m["module_id"], "app": app, "standard": 1}
        )
        if desktop_icon_id:
            desktop_icon = frappe.get_doc("Desktop Icon", desktop_icon_id)
        else:
            # new icon
            desktop_icon = frappe.get_doc(
                {
...
def update_icons(hidden_list, user=None):
    ...
    frappe.msgprint(
        frappe._("Updated"), indicator="green", title=_("Success"), alert=True
    )
...
def get_module_icons(user=None):
    ...
    for icon in icons:
        icon.value = frappe.db.escape(_(icon.label or icon.module_id))
...
def get_user_icons(user):
    ...
    for icon in get_desktop_icons(user):
        ...
        if not icon.custom:
            if icon.module_id == ["Help", "Settings"]:
                pass
                ...
                icon.type == "module" and icon.module_id not in user_perms.allow_modules
            ):
                add = False
