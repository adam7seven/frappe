...
def get_desktop_icons(user=None):
    ...
    if not user_icons:
        ...
        user_icon_ids = [icon.module_name for icon in user_icons]
        for standard_icon in standard_icons:
            if standard_icon.module_name not in user_icon_ids:
        ...
        if module_name not in ("Explore",):
            if user:
                ...
                if id:
                    ...
                    id = add_user_icon(module_name, standard=1)
                    icon = frappe.get_doc("Desktop Icon", id)
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
    ...
    for module_name in list(visible_list):
        id = frappe.db.get_value("Desktop Icon", {"module_name": module_name})
        if id:
...
def get_user_copy(module_name, user=None):
    ...
    :param module_name: ID of the module
    ...
    for icon in get_desktop_icons(user):
        ...
        if not icon.custom:
            if icon.module_name == ["Help", "Settings"]:
                ...
                icon.type == "module"
                and icon.module_name not in user_perms.allow_modules
            ):
                add = False
