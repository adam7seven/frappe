...
class TestEvent(IntegrationTestCase):
    ...
    def test_assign(self):
        ...
        self.assertEqual(
            set(json.loads(ev._assign)), {"test@example.com", self.test_user}
        )
    ...
    def test_recurring(self):
        ...
        ev_list = get_events(
            "2014-02-01", "2014-02-01", "Administrator", for_reminder=True
        )
        self.assertTrue(bool(list(filter(lambda e: e.id == ev.id, ev_list))))

        ev_list1 = get_events(
            "2015-01-20", "2015-01-20", "Administrator", for_reminder=True
        )
        self.assertFalse(bool(list(filter(lambda e: e.id == ev.id, ev_list1))))

        ev_list2 = get_events(
            "2014-02-20", "2014-02-20", "Administrator", for_reminder=True
        )
