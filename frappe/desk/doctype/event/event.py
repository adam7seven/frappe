...
class Event(Document):
    ...
    def on_trash(self):
        communications = frappe.get_all(
            "Communication", dict(reference_doctype=self.doctype, reference_id=self.id)
        )
        if communications:
            for communication in communications:
                frappe.delete_doc_if_exists("Communication", communication.id)
        ...
        if self.event_participants:
            for participant in self.event_participants:
                ...
                if comms := frappe.get_all(
                    "Communication", filters=filters, fields=["id"], distinct=True
                ):
                    for comm in comms:
    ...
    def update_communication(self, participant, communication):
        ...
        communication.communication_medium = (
            communication_mapping.get(self.event_category)
    ...
    def set_participants_email(self):
        for participant in self.event_participants:
            ...
            participant.email = (
                frappe.get_value("Contact", participant_contact, "email_id")
...
def delete_communication(event, reference_doctype, reference_docid):
    ...
    filters = [
        ...
        [
            ...
            "link_doctype",
            "=",
            deleted_participant.reference_doctype,
        ],
...
def get_events(
    ...
    events = frappe.db.sql(
        ...
        ORDER BY `tabEvent`.starts_on""".format(
            ...
            reminder_condition=(
                "AND coalesce(`tabEvent`.send_reminder, 0)=1" if for_reminder else ""
            ),
        ),
        {
            "start": start,
    ...
    def add_event(e, date):
        ...
        enddate = (
            add_days(
                date,
                int(
                    date_diff(e.ends_on.split(" ", 1)[0], e.starts_on.split(" ", 1)[0])
                ),
            )
        ...
        for participation in participations:
            if delete_event:
                ...
                total_participants = frappe.get_all(
                    "Event Participants",
                    filters={"parenttype": "Event", "parent": participation.parent},
                )
