...
frappe.ui.form.on("System Health Report", {
    ...
    refresh(frm) {
        ...
        frappe.realtime.on("pong", () => {
            ...
            frm.set_value("socketio_transport_mode", frappe.realtime.socket.io?.engine?.transport?.id);
        });
    ...
    setup_highlight(frm) {
        ...
        const conditions = {
            ...
            unhandled_emails: val => val > 3 && frm.doc.handled_emails > 3 && val / frm.doc.handled_emails > 0.05,
            failed_emails: val =>
