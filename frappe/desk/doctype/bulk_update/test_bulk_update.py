...
class TestBulkUpdate(IntegrationTestCase):
    ...
    def test_bulk_submit_in_background(self):
        unsubmitted = frappe.get_all(
            self.doctype, {"docstatus": 0}, limit=5, pluck="name"
        )
        failed = submit_cancel_or_update_docs(
            self.doctype, unsubmitted, action="submit"
        )
        ...
        unsubmitted = frappe.get_all(
            self.doctype, {"docstatus": 0}, limit=20, pluck="name"
        )
        ...
        submitted = frappe.get_all(
            self.doctype, {"docstatus": 1}, limit=20, pluck="name"
        )
