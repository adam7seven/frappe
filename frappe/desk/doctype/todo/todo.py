...
class ToDo(Document):
    ...
    def validate(self):
        ...
        if self.is_new():
            if self.assigned_by == self.allocated_to:
                ...
                ).format(get_fullname(self.assigned_by), self.description)
            else:
    ...
    def add_assign_comment(self, text, comment_type):
        ...
        frappe.get_doc(self.reference_type, self.reference_id).add_comment(
            comment_type, text
        )
    ...
    def delete_communication_links(self):
        ...
        return frappe.db.delete(
            "Communication Link", {"link_doctype": self.doctype, "link_id": self.id}
        )
    ...
    def get_owners(cls, filters=None):
        ...
        rows = frappe.get_all(
            cls.DocType, filters=filters or {}, fields=["allocated_to"]
        )
