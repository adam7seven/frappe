...
def get_tagged_docs(doctype, tag):
    ...
    return (
        frappe.qb.from_(doctype).where(doctype._user_tags.like(tag)).select(doctype.id)
    ).run()
...
def get_tags(doctype, txt):
    ...
    return sorted(
        filter(lambda t: t and txt.casefold() in t.casefold(), list(set(tags)))
    )
...
def update_tags(doc, tags):
    ...
    existing_tags = [
        ...
        for tag in frappe.get_list(
            "Tag Link",
            filters={"document_type": doc.doctype, "document_id": doc.id},
            fields=["tag"],
        )
    ]
    ...
    for tag in deleted_tags:
        frappe.db.delete(
            "Tag Link",
            {"document_type": doc.doctype, "document_id": doc.id, "tag": tag},
        )
...
def get_documents_for_tag(tag):
    ...
    result = frappe.get_list(
        "Tag Link",
        filters={"tag": tag},
        fields=["document_type", "document_id", "title", "tag"],
    )
