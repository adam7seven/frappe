...
frappe.ui.form.on("Workspace", {
    ...
    refresh: function (frm) {
        ...
        let url = `/app/${
            frm.doc.public ? frappe.router.slug(frm.doc.title) : "private/" + frappe.router.slug(frm.doc.title)
        }`;
        ...
        if (
            frm.doc.for_user ||
            (frm.doc.public && !frm.has_perm("write") && !frappe.user.has_role("Workspace Manager"))
        ) {
