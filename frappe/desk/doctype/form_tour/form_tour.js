...
frappe.ui.form.on("Form Tour", {
    ...
    async before_save(frm) {
        if (
            ...
            frappe.throw(__("Referance Doctype and Dashboard Name both can't be used at the same time."));
        }
...
async function get_path(frm) {
    ...
    switch (route[0]) {
        ...
        case "List":
            ...
            if (!frm.doc.reference_doctype) {
                if (frm.doc.list_name == "Dashboard") return ["dashboard-view", frm.doc.dashboard_name || "*"];
                route.push("*");
            } else {
