...
def get_permission_query_conditions(user):
    ...
    if allowed_reports:
        report_condition = (
            "`tabDashboard Chart`.`report_id` in ({allowed_reports})".format(
    ...
    if data and group_by_field_field.fieldtype == "Link":  # if @group_by_field is link
        ...
        if title_field.title_field:  # if has title_field
            for item in data:  # replace chart labels from id to title value
                item.id = frappe.get_value(
                    group_by_field_field.options, item.id, title_field.title_field
                )
...
class DashboardChart(Document):
    ...
    if TYPE_CHECKING:
        ...
        timeseries: DF.Check
        timespan: DF.Literal[
            "Last Year", "Last Quarter", "Last Month", "Last Week", "Select Date Range"
        ]
    ...
    def on_update(self):
        ...
        if frappe.conf.developer_mode and self.is_standard:
            export_to_files(
                record_list=[["Dashboard Chart", self.id]], record_module=self.module
            )
    ...
    def check_required_field(self):
        ...
        if (
            ...
            frappe.throw(
                _("Parent document type is required to create a dashboard chart")
            )
