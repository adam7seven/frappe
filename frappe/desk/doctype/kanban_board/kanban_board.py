...
def update_order_for_single_card(
    board_id, docid, from_colname, to_colname, old_index, new_index
):
    """Save the order of cards in columns"""
    board = frappe.get_doc("Kanban Board", board_id)
    doctype = board.reference_doctype
...
def get_order_for_column(board, colname):
    ...
    return frappe.as_json(
        frappe.get_list(board.reference_doctype, filters=filters, pluck="id")
    )
