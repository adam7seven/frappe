...
def get_title(doctype, docid, title_field=None):
    ...
    return (
        docid
        if title_field == "id"
        else frappe.db.get_value(doctype, docid, title_field)
    )
...
def _get_user_ids(user_emails):
    user_ids = frappe.db.get_values(
        "User", {"enabled": 1, "email": ("in", user_emails)}, "id", pluck=True
    )
    return [user for user in user_ids if is_notifications_enabled(user)]
...
def send_notification_email(doc: NotificationLog):
    ...
    user = frappe.db.get_value(
        "User", doc.for_user, fieldname=["email", "language"], as_dict=True
    )
    if not user:
        return
...
def get_email_header(doc, language: str | None = None):
    ...
    header_map = {
        ...
        "Energy Point": _("Energy Point Update on {0}", lang=language).format(docid),
    }
...
def mark_all_as_read():
    ...
    if unread_docids:
        ...
        frappe.db.set_value(
            "Notification Log", filters, "read", 1, update_modified=False
        )
...
def mark_as_read(docid: str):
    ...
    if docid:
        frappe.db.set_value(
            "Notification Log", str(docid), "read", 1, update_modified=False
        )
...
def set_notifications_as_unseen(user):
    try:
        frappe.db.set_value(
            "Notification Settings", user, "seen", 0, update_modified=False
        )
