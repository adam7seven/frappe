...
class DashboardChartSource(Document):
    ...
    def on_update(self):
        ...
        if not frappe.conf.developer_mode:
            frappe.throw(
                _("Creation of this document is only permitted in developer mode.")
            )

        export_to_files(
            record_list=[[self.doctype, self.name]],
    ...
    def get_folder_path(self) -> Path:
        ...
        return (
            Path(get_module_path(self.module)) / FOLDER_NAME / frappe.scrub(self.name)
        )
