    ...
    def validate(self):
        if self.type == "Document Type":
            if not (self.document_type and self.function):
                frappe.throw(
                    _("Document Type and Function are required to create a number card")
                )
            ...
            if (
                ...
                frappe.throw(
                    _("Parent Document Type is required to create a number card")
                )
...
def get_permission_query_conditions(user=None):
    ...
    if allowed_doctypes:
        doctype_condition = (
            "`tabNumber Card`.`document_type` in ({allowed_doctypes})".format(
...
def get_cards_for_user(doctype, txt, searchfield, start, page_len, filters):
    ...
    if txt:
        search_conditions = [
            numberCard[field].like(f"%{txt}%") for field in searchfields
        ]
