...
def get_report_doc(report_id):
    ...
    if doc.disabled:
        frappe.throw(_("Report {0} is disabled").format(report_id))
...
def get_report_result(report, filters):
    ...
    if report.report_type == "Query Report":
        ...
        ref_report = get_report_doc(report.report_id)
...
def get_script(report_id):
    ...
    if not script:
        script = "frappe.query_reports['%s']={}" % report_id

    return {
        "script": render_include(script),
        "html_format": html_format,
        "execution_time": frappe.cache.hget("report_execution_time", report_id) or 0,
        "filters": report.filters,
        "custom_report_id": report.id if report.get("is_custom_report") else None,
    }
...
def run(
    ...
    validate_filters_permissions(report_id, filters, user)
    report = get_report_doc(report_id)
    ...
    try:
        if (
            ...
            if filters:
                ...
                dn = filters.pop("prepared_report_id", None)
            else:
                dn = ""
...
def export_query():
    ...
    report_id = form_params.report_id
    frappe.permissions.can_export(
        frappe.get_cached_value("Report", report_id, "ref_doctype"),
        raise_exception=True,
    )
...
@frappe.whitelist()
def save_report(reference_report, report_id, columns, filters):
    ...
    if docid:
        ...
        frappe.msgprint(_("{0} saved successfully").format(new_report.id))
...
def validate_filters_permissions(report_id, filters=None, user=None):
    if not filters:
        return
