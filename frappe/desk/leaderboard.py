...
def get_energy_point_leaderboard(date_range, company=None, field=None, limit=None):
    ...
    energy_point_users = frappe.get_all(
        "Energy Point Log",
        fields=["user as id", "sum(points) as value"],
        filters=filters,
        group_by="user",
        order_by="value desc",
    )

    energy_point_users_list = list(map(lambda x: x["id"], energy_point_users))
    for user in users:
        if user not in energy_point_users_list:
            energy_point_users.append({"id": user, "value": 0})

    for user in energy_point_users:
        user_id = user["id"]
        user["id"] = get_fullname(user["id"])
        user["formatted_id"] = (
