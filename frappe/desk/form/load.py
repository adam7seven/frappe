...
@frappe.whitelist()
def getdoc(doctype, id, user=None):
    """
...
def get_versions(doc: "Document") -> list[dict]:
    ...
    return frappe.get_all(
        "Version",
        filters=dict(ref_doctype=doc.doctype, docid=doc.id),
...
def get_document_email(doctype, id):
    ...
    email = email.split("@")
    return f"{email[0]}+{quote(doctype)}={quote(cstr(id))}@{email[1]}"
...
def get_title_values_for_link_and_dynamic_link_fields(doc, link_fields=None):
    ...
    for field in link_fields:
        link_docid = getattr(doc, field.fieldname, None)
