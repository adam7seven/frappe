...
def update_follow(doctype: str, doc_id: str, following: bool):
    if following:
        return follow_document(doctype, doc_id, frappe.session.user)
    else:
...
def unfollow_document(doctype, doc_id, user):
    ...
    if doc:
        frappe.delete_doc("Document Follow", doc[0].id)
        return 1
    return 0
...
def get_version(doctype, doc_id, frequency, user):
    ...
    if version:
        for v in version:
            ...
            timeline_items = []
            if change.changed:
                timeline_items = get_field_changed(
                    change.changed, time, doctype, doc_id, v
                )
            if change.row_changed:
                timeline_items = get_row_changed(
                    change.row_changed, time, doctype, doc_id, v
                )
            if change.added:
...
def is_document_followed(doctype, doc_id, user):
    return frappe.db.exists(
        "Document Follow", {"ref_doctype": doctype, "ref_docid": doc_id, "user": user}
    )
...
def get_follow_users(doctype, doc_id):
    return frappe.get_all(
        "Document Follow",
        filters={"ref_doctype": doctype, "ref_docid": doc_id},
        fields=["user"],
    )
