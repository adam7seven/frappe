...
def get_submitted_linked_docs(
    doctype: str, name: str, ignore_doctypes_on_cancel_all=None
) -> list[tuple]:
...
def get_referencing_documents(
    ...
    if not link_info.get("is_child"):
        ...
        return {
            from_table: frappe.get_all(from_table, filters, pluck="name", order_by=None)
        }
...
def get_linked_docs(
    doctype: str, name: str, linkinfo: dict | None = None
) -> dict[str, list]:
    ...
    for linked_doctype, link_context in linkinfo.items():
        ...
        if filters_ctx := link_context.get("filters"):
            ...
            or_filters = [
                [child_doctype, link_fieldnames, "=", name]
            ...
            or_filters = [
                [linked_doctype, fieldname, "=", name] for fieldname in link_fieldnames
            ]
            # dynamic link_context
            if doctype_fieldname := link_context.get("doctype_fieldname"):
