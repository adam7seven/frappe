...
def add(args=None, *, ignore_permissions=False):
    ...
    description = escape_html(
        args.get(
            "description",
            _("Assignment for {0} {1}").format(args["doctype"], args["id"]),
        )
    )
    ...
    for assign_to in frappe.parse_json(args.get("assign_to")):
        ...
        if frappe.get_all("ToDo", filters=filters):
            ...
            if frappe.get_meta(args["doctype"]).get_field("assigned_to"):
                frappe.db.set_value(
                    args["doctype"], args["id"], "assigned_to", assign_to
                )
...
def close_all_assignments(doctype, id, ignore_permissions=False):
    assignments = frappe.get_all(
        ...
        filters=dict(
            reference_type=doctype, reference_id=id, status=("!=", "Cancelled")
        ),
    )
    if not assignments:
        return False
...
def set_status(
    doctype, id, todo=None, assign_to=None, status="Cancelled", ignore_permissions=False
):
    """remove from todo"""
    ...
    if frappe.get_meta(doctype).get_field("assigned_to") and status in (
        "Cancelled",
        "Closed",
    ):
...
def notify_assignment(
    assigned_by, allocated_to, doc_type, doc_id, action="CLOSE", description=None
):
    """
    ...
    user_id = frappe.get_cached_value("User", frappe.session.user, "full_name")
    ...
    if action == "CLOSE":
        ...
        ).format(frappe.bold(_(doc_type)), get_title_html(title), frappe.bold(user_id))
    else:
        user_id = frappe.bold(user_id)
        ...
        ).format(user_id, document_type, title)
