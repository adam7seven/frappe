...
class Workspace:
    ...
    def is_item_allowed(self, name, item_type):
        ...
        if item_type == "doctype":
            return name in (self.can_read or []) and name in (
                self.restricted_doctypes or []
            )
        if item_type == "page":
    ...
    def is_custom_block_permitted(self, custom_block_name):
        ...
        allowed = [
            ...
            for d in frappe.get_all(
                "Has Role", fields=["role"], filters={"parent": custom_block_name}
            )
        ]
    ...
    def get_custom_blocks(self):
        ...
        if frappe.has_permission("Custom HTML Block", throw=False):
            ...
            for custom_block in custom_blocks:
                if frappe.has_permission(
                    "Custom HTML Block", doc=custom_block.custom_block_name
                ):
                    if not self.is_custom_block_permitted(
                    ...
                    custom_block.label = (
                        ...
                        else _(custom_block.custom_block_name)
                    )
...
def get_workspace_sidebar_items():
    ...
    try:
        workspace_visibilty = loads(
            frappe.db.get_single_value(
                "Workspace Settings", "workspace_visibility_json"
            )
            or "{}"
        )
    ...
    for page in all_pages:
        try:
            ...
            if not page["app"] and page["module"]:
                page["app"] = frappe.db.get_value(
                    "Module Def", page["module"], "app_name"
                ) or get_module_app(page["module"])
    ...
    if len(pages) == 0:
        pages.append(
            next((x for x in all_pages if x["title"] == "Welcome Workspace"), None)
        )
