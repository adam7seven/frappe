...
def get_notifications():
    ...
    out["open_count_doctype"] = get_notifications_for_doctypes(
        ...
        if d in can_read:
            ...
            if d in notification_count:
                ...
                try:
                    ...
                    frappe.cache.hset(
                        "notification_count:" + d, frappe.session.user, result
                    )
...
def clear_notifications(user=None):
    ...
    for id in groups:
        if user:
            frappe.cache.hdel("notification_count:" + id, user)
        else:
            frappe.cache.delete_key("notification_count:" + id)
                        ...
                        if len(subscribed_documents) > 0:
                            ...
                            for document in subscribed_documents:
                                if key_config.get(document):
                                    subscribed_docs_config[document] = key_config.get(
                                        document
                                    )
...
def get_open_count(doctype: str, id: str, items=None):
    ...
    for d in items:
        ...
        if internal_link_for_doctype:
            internal_links_data_for_d = get_internal_links(
                doc, internal_link_for_doctype, d
            )
            if internal_links_data_for_d["count"]:
                ...
                try:
                    ...
                    out["external_links_found"].append(
                        {"doctype": d, "open_count": 0, "count": 0}
                    )
        else:
            external_links_data_for_d = get_external_links(d, id, links)
            out["external_links_found"].append(external_links_data_for_d)
...
def get_external_links(doctype, id, links):
    fieldname = links.get("non_standard_fieldnames", {}).get(
        doctype, links.get("fieldname")
    )
