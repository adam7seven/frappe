...
def get_email_subject_for_2fa(kwargs_dict):
    ...
    subject_template = _("Login Verification Code from {}").format(frappe.get_system_settings("otp_issuer_name"))
...
def get_email_body_for_qr_code(kwargs_dict):
    ...
    body_template = _("Please click on the following link and follow the instructions on the page. {0}").format(
        "<br><br> <a href='{{qrcode_link}}'>{{qrcode_link}}</a>"
    )
    return frappe.render_template(body_template, kwargs_dict)
