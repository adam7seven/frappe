...
def two_factor_is_enabled(user=None):
    ...
    enabled = int(
        frappe.db.get_single_value("System Settings", "enable_two_factor_auth") or 0
    )
    if enabled:
        bypass_two_factor_auth = int(
            frappe.db.get_single_value(
                "System Settings", "bypass_2fa_for_retricted_ip_users"
            )
            or 0
        )
...
def get_link_for_qrcode(user, totp_uri):
    ...
    key_uri = f"{key}_uri"
    lifespan = (
        int(frappe.db.get_single_value("System Settings", "lifespan_qrcode_image"))
        or 240
    )
...
def should_remove_barcode_image(barcode):
    ...
    lifespan = (
        frappe.db.get_single_value("System Settings", "lifespan_qrcode_image") or 240
    )
