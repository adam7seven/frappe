...
class DatabaseQuery:
    ...
    def set_optional_columns(self):
        ...
        self.filters[:] = [f for f in self.filters if f.fieldname not in optional_fields or f.fieldname in self.columns]
    ...
    def prepare_filter_condition(self, ft: FilterTuple) -> str:
        ...
        if f.operator.lower() in ("in", "not in"):
            ...
            if df and (
                df.fieldtype in ("Check", "Float", "Int", "Currency", "Percent") or getattr(df, "not_nullable", False)
            ):
                can_be_null = False
        ...
        for df in doctype_link_fields:
            ...
            if user_permission_values:
                ...
                if frappe.get_system_settings("apply_strict_user_permissions"):
                    ...
                    empty_value_condition = cast_id(f"ifnull(`tab{self.doctype}`.`{df.get('fieldname')}`, '')=''")
