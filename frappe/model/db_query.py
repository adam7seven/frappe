...
ORDER_BY_PATTERN = re.compile(
    r"\ order\ by\ |\ asc|\ ASC|\ desc|\ DESC", flags=re.IGNORECASE
)
...
class DatabaseQuery:
    ...
    def execute(
        ...
        *,
        ...
        if (
            as_list
            and not isinstance(self.fields, (Sequence | str))
            ...
            if "." in field:
                ...
                if linked_field.fieldtype == "Link":
                    linked_table = self.append_link_table(
                        linked_doctype, linked_fieldname
                    )
        ...
        for field in self.fields:
            ...
            if SUB_QUERY_PATTERN.match(field):
                ...
                if function in blacklisted_functions:
                    frappe.throw(
                        _("Use of function {0} in field is restricted").format(
                            function
                        ),
        ...
        if len(self.tables) > 1 or len(self.link_tables) > 0:
            for idx, field in enumerate(self.fields):
                if (
                    field is not None
                    and "." not in field
                    and not _in_standard_sql_methods(field)
                ):
    ...
    def set_optional_columns(self):
        ...
        self.fields[:] = [
            f for f in self.fields if f not in optional_fields or f in self.columns
        ]
        self.filters[:] = [
            f
            for f in self.filters
            if f.fieldname not in optional_fields or f.fieldname in self.columns
        ]
    ...
    def build_filter_conditions(
        self, filters: Filters, conditions: list, ignore_permissions=None
    ):
        ...
        for i, field in enumerate(self.fields):
            ...
            # handle child / joined table fields
                ...
                if wrap_grave_quotes(table) in self.query_tables:
                    ...
                    if (
                        column in permitted_child_table_fields
                        or column in optional_fields
                    ):
                        continue
                    else:
                        self.remove_field(i)
    ...
    def prepare_filter_condition(self, ft: FilterTuple) -> str:
        ...
        if f.operator.lower() in ("in", "not in"):
            ...
            if value is None:
                ...
                fallback = "''"
                value = [
                    frappe.db.escape((cstr(v) or "").strip(), percent=False)
                    for v in values
                ]
                if len(value):
                    value = f"({', '.join(value)})"
        ...
        if (
            ...
            if (
                f.operator.lower() == "like"
                and frappe.conf.get("db_type") == "postgres"
            ):
                f.operator = "ilike"
    ...
    def build_match_conditions(self, as_condition=True) -> str | list:
        ...
        if as_condition:
            ...
            if doctype_conditions:
                conditions += (
                    (" and " + doctype_conditions) if conditions else doctype_conditions
                )
        ...
        for df in doctype_link_fields:
            ...
            if user_permission_values:
                ...
                if docs:
                    ...
                    condition += (
                        cast_id(f"`tab{self.doctype}`.`{df.get('fieldname')}`")
                        + f" in ({values})"
                    )
    ...
    def get_permission_query_conditions(self) -> str:
        ...
        for method in condition_methods:
            if c := frappe.call(
                frappe.get_attr(method), self.user, doctype=self.doctype
            ):
                conditions.append(c)

        if (
            permission_script_id := get_server_script_map()
        ...
        if self.order_by and self.order_by != "KEEP_DEFAULT_ORDERING":
            ...
            if not group_function_without_group_by:
                ...
                if self.doctype_meta.sort_field and "," in self.doctype_meta.sort_field:
                    ...
                    if self.order_by:
                        args.order_by = f"`tab{self.doctype}`.`{sort_field or 'creation'}` {sort_order or 'desc'}"
...
def check_parent_permission(parent, child_doctype):
    if parent:
        ...
        if child_doctype and not (
            ...
            or frappe.db.exists(
                "Custom Field", {"dt": parent, "options": child_doctype}
            )
        ):
...
def get_between_date_filter(value, df=None):
    ...
    if fieldtype == "Datetime":
        from_date = _convert_type_for_between_filters(
            from_date, set_time=datetime.time()
        )
