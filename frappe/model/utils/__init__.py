...
def set_field_property(filters, key, value):
    ...
    docs = [frappe.get_doc("DocType", d.parent) for d in frappe.get_all("DocField", fields=["parent"], filters=filters)]
        ...
        if "{% include" in content:
            ...
            for path in paths:
                ...
                with open(frappe.get_app_path(app, app_path), encoding="utf-8") as f:
                    ...
                    content = re.sub(rf"""{{% include\s['"]{path}['"]\s%}}""", include.replace("\\", "\\\\"), content)
...
def get_fetch_values(doctype, fieldname, value):
    ...
    fields_to_fetch = {df.fieldname: df.fetch_from.split(".", 1)[1] for df in meta.get_fields_to_fetch(fieldname)}
