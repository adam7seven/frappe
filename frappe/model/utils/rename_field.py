...
def rename_field(doctype, old_fieldname, new_fieldname, validate=True):
    ...
    if new_field.fieldtype in table_fields:
        ...
        if meta.issingle:
            ...
            # copy field value
            frappe.db.sql(
                f"""update `tab{doctype}` set `{new_fieldname}`=`{old_fieldname}`"""
            )
...
def update_reports(doctype, old_fieldname, new_fieldname):
    ...
    for r in reports:
        ...
        if report_dict.get("updated"):
            ...
            frappe.db.sql(
                """update `tabReport` set `json`=%s where id=%s""", (new_val, r.id)
            )
...
def update_user_settings(doctype, old_fieldname, new_fieldname):
    ...
    for user_setting in user_settings:
        update_user_settings_data(
            user_setting, "docfield", old_fieldname, new_fieldname
        )
