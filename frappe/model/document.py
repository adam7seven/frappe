...
class Document(BaseDocument, DocRef):
    ...
    def update_child_table(self, fieldname: str, df: Optional["DocField"] = None):
        ...
        if not (
            ...
            existing_row_names = [
                row.name for row in all_rows if row.name and not row.is_new()
            ]
    ...
    def check_if_latest(self):
        ...
        if cstr(previous.modified) != cstr(self._original_modified):
            frappe.msgprint(
                _(
                    f"Error: {self.name} ({self.doctype}) has been modified after you have opened it"
                )
    ...
    def rename(self, name: str, merge=False, force=False, validate_rename=True):
        ...
        return self._rename(
            name=name, merge=merge, force=force, validate_rename=validate_rename
        )
    ...
    def get_signature(self):
        ...
        return hashlib.sha224(
            f"{self.doctype}:{self.name}".encode(), usedforsecurity=False
        ).hexdigest()
