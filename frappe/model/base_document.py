...
class BaseDocument:
	...
	def _init_child(self, value, key, set_defaults: bool = False):
		...
		# set default values for child table
		if set_defaults:
			df = self.meta.get_field(key)
			nd = frappe.new_doc(df.options, parent_doc=self, parentfield=df.fieldname, as_dict=True)
			value.update_if_missing(nd)
			...
			if docid:
				...
				if not meta.get("is_virtual"):
					if not fields_to_fetch:
						...
						values = (
							frappe.db.get_value(doctype, docid, values_to_fetch, as_dict=True) or empty_values
						)
