...
class Meta(Document):
    ...
    def get_global_search_fields(self):
        ...
        if getattr(self, "show_name_in_global_search", None):
            fields.append(
                frappe._dict(fieldtype="Data", fieldname="name", label="Name")
            )
    ...
    def _valid_fields(self):
        ...
        ) and self.name in self.special_doctypes:
    ...
    def add_custom_links_and_actions(self):
        for doctype, fieldname in (
            ...
            for d in frappe.get_all(
                doctype,
                fields="*",
                filters=dict(parent=self.name, custom=1),
                ignore_ddl=True,
            ):
    ...
    def check_if_large_table(self):
        ...
        if frappe.db.estimate_count(self.name) > LARGE_TABLE_SIZE_THRESHOLD:
            recent_change = frappe.db.get_value(
                self.name, {}, "creation", order_by="creation desc"
            )
    ...
    def get_permitted_fieldnames(
        ...
        *,
        ...
        if not permission_type:
            permission_type = (
                "select"
                if frappe.only_has_select_perm(self.name, user=user)
                else "read"
            )
        ...
        if 0 not in permlevel_access and permission_type in ("read", "select"):
            if frappe.share.get_shared(
                self.name, user, rights=[permission_type], limit=1
            ):
