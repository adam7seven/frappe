    ...
    for df in frappe.get_meta(doc["doctype"]).get("fields"):
        if df.get("default"):
            if cstr(df.default).startswith(":"):
                default_value = get_default_based_on_another_field(
...
def get_default_based_on_another_field(df, user_permissions, parent_doc):
    ...
    is_allowed_default_value = not user_permissions_exist(
        ...
        default_value
        in get_allowed_docs_for_doctype(user_permissions[df.options], df.parent)
    )
