...
def sync_for(app_name, force=0, reset_permissions=False):
    ...
    if app_name == "frappe":
        ...
        for core_module in [
            ...
            files.append(
                os.path.join(
                    FRAPPE_PATH, "core", "doctype", core_module, f"{core_module}.json"
                )
            )
        ...
        for desk_module in [
            ...
            files.append(
                os.path.join(
                    FRAPPE_PATH, "desk", "doctype", desk_module, f"{desk_module}.json"
                )
            )
