...
def get_workflow_id(doctype):
    ...
    if workflow_id is None:
        workflow_id = frappe.db.get_value(
            "Workflow", {"document_type": doctype, "is_active": 1}, "id"
        )
...
def bulk_workflow_approval(docids, doctype, action):
    ...
    if len(docids) < 20:
        ...
        frappe.msgprint(
            _("Bulk {0} is enqueued in background.").format(action), alert=True
        )
        frappe.enqueue(
        ...
        frappe.throw(
            _("Bulk approval only support up to 500 documents."),
        ...
        try:
            ...
            if not message_dict:
                if frappe.message_log:
                    ...
                    successful_transactions[docid].append(
                        {"docid": docid, "message": None}
                    )
