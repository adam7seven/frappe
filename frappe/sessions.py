...
def get_expired_sessions():
    ...
    return (frappe.qb.from_(sessions).select(sessions.sid).where(sessions.lastupdate < get_expired_threshold())).run(
        pluck=True
    )
...
class Session:
    ...
    def __init__(
        ...
        self.sid = cstr(frappe.form_dict.pop("sid", None) or unquote(frappe.request.cookies.get("sid", "Guest")))
        self.user = user
    ...
    def get_session_data_from_cache(self):
        ...
        if data:
            ...
            self.time_diff = frappe.utils.time_diff_in_seconds(frappe.utils.now(), session_data.get("last_updated"))
    ...
    def update(self, force=False):
        ...
        updated_in_db = False
        if (force or (time_diff is None) or (time_diff > 600) or self._update_in_cache) and not frappe.flags.read_only:
