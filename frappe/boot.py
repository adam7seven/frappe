...
def get_user_pages_or_reports(parent, cache=False):
    ...
    subq = frappe.qb.from_(customRole).select(customRole[parent.lower()]).where(customRole[parent.lower()].isnotnull())
    ...
    no_of_roles = SubQuery(frappe.qb.from_(hasRole).select(Count("*")).where(hasRole.parent == parentTable.id))

    # pages and reports with no role are allowed
    rows_with_no_roles = (
        frappe.qb.from_(parentTable).select(parentTable.id, parentTable.modified, *columns).where(no_of_roles == 0)
    ).run(as_dict=True)
