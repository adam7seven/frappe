...
def get_allowed_report_ids(cache=False) -> set[str]:
    return {cstr(report) for report in get_allowed_reports(cache).keys() if report}


def get_user_pages_or_reports(parent, cache=False):
    if cache:
        has_role = frappe.cache.get_value(
            "has_role:" + parent, user=frappe.session.user
        )
        if has_role:
            return has_role
    ...
    pages_with_custom_roles = (
        ...
        .from_(parentTable)
        .select(
            customRole[parent.lower()].as_("id"),
            customRole.modified,
    ...
    pages_with_standard_roles = (
        ...
        .from_(parentTable)
        .select(parentTable.name.as_("id"), parentTable.modified, *columns)
        .where(
            (hasRole.role.isin(roles))
    ...
    if is_report:
        pages_with_standard_roles = pages_with_standard_roles.where(
            report.disabled == 0
        )
    ...
    no_of_roles = SubQuery(
        frappe.qb.from_(hasRole)
        .select(Count("*"))
        .where(hasRole.parent == parentTable.id)
    )
...
def get_link_preview_doctypes():
    ...
    link_preview_doctypes = [
        d.id for d in frappe.get_all("DocType", {"show_preview_popup": 1})
    ]
...
def add_layouts(bootinfo):
    ...
    bootinfo.doctype_layouts = frappe.get_all(
        "DocType Layout", ["id", "route", "document_type"]
    )
