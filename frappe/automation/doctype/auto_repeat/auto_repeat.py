...
class AutoRepeat(Document):
    ...
    def validate_submit_on_creation(self):
        if self.submit_on_creation and not frappe.get_meta(self.reference_doctype).is_submittable:
            frappe.throw(
                _("Cannot enable {0} for a non-submittable doctype").format(frappe.bold(_("Submit on Creation")))
            )
    ...
    def validate_dates(self):
        ...
        if self.end_date == self.start_date:
            frappe.throw(
                _("{0} should not be same as {1}").format(frappe.bold(_("End Date")), frappe.bold(_("Start Date")))
            )
    ...
    def create_documents(self):
        try:
            ...
            error_log = self.log_error(_("Auto repeat failed. Please enable auto repeat after fixing the issues."))
    ...
    def send_notification(self, new_doc):
        ...
        try:
            attachments = [
                frappe.attach_print(new_doc.doctype, new_doc.name, file_name=new_doc.name, print_format=print_format)
            ]
