...
class TestAutoRepeat(IntegrationTestCase):
	...
	def test_daily_auto_repeat(self):
        ...
        new_todo = frappe.db.get_value(
            "ToDo", {"auto_repeat": doc.id, "id": ("!=", todo.id)}, "id"
        )
	...
	def test_email_notification(self):
        ...
        email_queue = frappe.db.exists(
            "Email Queue", dict(reference_doctype="ToDo", reference_id=new_todo)
        )
    ...
    return frappe.get_doc(
        {
            ...
            or frappe.db.get_value("ToDo", "id"),
