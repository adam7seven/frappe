...
class TestAutoAssign(IntegrationTestCase):
    ...
    def test_round_robin(self):
        ...
        self.assertEqual(
            frappe.db.get_value(
                "ToDo",
                dict(
                    reference_type=TEST_DOCTYPE, reference_id=record.id, status="Open"
                ),
                "allocated_to",
            ),
            "test@example.com",
    ...
    def test_load_balancing(self):
        ...
        for user in ("test@example.com", "test1@example.com", "test2@example.com"):
            self.assertEqual(
                len(
                    frappe.get_all(
                        "ToDo", dict(allocated_to=user, reference_type=TEST_DOCTYPE)
                    )
                ),
                10,
            )
        ...
        for d in frappe.get_all(
            "ToDo",
            dict(reference_type=TEST_DOCTYPE, allocated_to="test@example.com"),
            limit=5,
        ):
        ...
        for test_user in ("test1@example.com", "test2@example.com"):
            ...
            self.assertEqual(
                frappe.db.get_value(
                    "ToDo",
                    dict(
                        reference_type=TEST_DOCTYPE, reference_id=note.id, status="Open"
                    ),
                    "owner",
                ),
                test_user,
    ...
    def test_assignment_rule_condition(self):
        ...
        note1_todo = frappe.get_all(
            "ToDo",
            filters=dict(
                reference_type=TEST_DOCTYPE, reference_id=note1.id, status="Open"
            ),
        )[0]
        ...
        self.assertEqual(
            frappe.utils.get_date_str(note1_todo_doc.date), note1.expiry_date
        )
        ...
        note2_todo = frappe.get_all(
            "ToDo",
            filters=dict(
                reference_type=TEST_DOCTYPE, reference_id=note2.id, status="Open"
            ),
            fields=["id", "date"],
        )[0]
        self.assertNotEqual(
            frappe.utils.get_date_str(note2_todo.date), note1.expiry_date
        )
