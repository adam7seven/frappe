...
class TestAutoAssign(IntegrationTestCase):
    ...
    def test_assign_condition(self):
        ...
        self.assertEqual(
            frappe.db.get_value(
                "ToDo",
                dict(
                    reference_type=TEST_DOCTYPE, reference_name=note.name, status="Open"
                ),
                "allocated_to",
            ),
            None,
    ...
    def test_assignment_rule_condition(self):
        ...
        note1_todo = frappe.get_all(
            "ToDo",
            filters=dict(
                reference_type=TEST_DOCTYPE, reference_name=note1.name, status="Open"
            ),
        )[0]
        ...
        note2_todo = frappe.get_all(
            "ToDo",
            filters=dict(
                reference_type=TEST_DOCTYPE, reference_name=note2.name, status="Open"
            ),
            fields=["name", "date"],
        )[0]
        self.assertNotEqual(
