...
class AssignmentRule(Document):
    ...
    def validate_document_types(self):
        if self.document_type == "ToDo":
            frappe.throw(_("Assignment Rule is not allowed on document type {0}").format(frappe.bold(_("ToDo"))))
...
def apply(doc=None, method=None, doctype=None, id=None):
    ...
    skip_assignment_rules = (
        frappe.flags.in_patch or frappe.flags.in_install or frappe.flags.in_setup_wizard or doctype in log_types
    )
