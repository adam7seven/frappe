...
class AssignmentRule(Document):
    ...
    def validate_document_types(self):
        if self.document_type == "ToDo":
            frappe.throw(
                _("Assignment Rule is not allowed on document type {0}").format(
                    frappe.bold(_("ToDo"))
                )
            )
        ...
        if len(set(assignment_days)) != len(assignment_days):
            frappe.throw(
                _("The following Assignment Days have been repeated: {0}").format(
                    comma_and(
                        [_(day) for day in get_repeated(assignment_days)],
                        add_quotes=False,
                    )
                )
            )
    ...
    def apply_unassign(self, doc, assignments):
        if self.unassign_condition and self.name in [
            d.assignment_rule for d in assignments
        ]:
        ...
        if user:
            assign_to.add(
                dict(
                    ...
                    notify=True,
                    date=(
                        doc.get(self.due_date_based_on)
    ...
    def safe_eval(self, fieldname, doc):
        try:
            ...
            frappe.msgprint(
                frappe._("Auto assignment failed: {0}").format(str(e)),
                indicator="orange",
            )
...
def apply(doc=None, method=None, doctype=None, name=None):
    ...
    assignment_rule_docs: list[AssignmentRule] = [
        frappe.get_cached_doc("Assignment Rule", d.get("name"))
