...
class TestEnergyPointLog(IntegrationTestCase):
    ...
    def test_review(self):
        ...
        self.assertEqual(
            energy_points_after_review,
            energy_points_before_review + appreciation_points,
        )
        self.assertEqual(
            review_points_after_review,
            review_points_before_review - appreciation_points,
        )
    ...
    def test_revert_points_on_cancelled_doc(self):
        ...
        energy_point_logs = frappe.get_all(
            "Energy Point Log", fields=["reference_id", "type", "reverted"]
        )

        self.assertListEqual(
            energy_point_logs,
            [
                {"reference_id": created_todo.id, "type": "Revert", "reverted": 0},
                {"reference_id": created_todo.id, "type": "Auto", "reverted": 1},
            ],
        )
    ...
    def test_point_allocation_for_assigned_users(self):
        ...
        assign_users_to_todo(todo.id, ["test@example.com", "test2@example.com"])
        ...
        self.assertEqual(
            test2_user_after_points, test2_user_before_points + rule.points
        )

    def test_eps_heatmap_query(self):
        # Just asserts that query works, not correctness.
        self.assertIsInstance(
            get_energy_points_heatmap_data(user="test@example.com", date=None), dict
        )
    ...
    def test_points_on_field_value_change(self):
        ...
        self.assertEqual(
            points_after_changing_todo_description,
    ...
    def test_apply_only_once(self):
        ...
        self.assertEqual(
            ...
            first_user_points + todo_point_rule.points,
        )
    ...
    def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
        ...
        log_id = frappe.db.exists("Energy Point Log", {"reference_id": created_todo.id})
        frappe.get_doc("Energy Point Log", log_id).revert("Just for test")
        ...
        self.assertEqual(
            points_after_reverting_todo, points_after_closing_todo - rule_points
        )
        self.assertEqual(
...
def create_energy_point_rule_for_todo(
    ...
    id = "ToDo Closed"
    point_rule_exists = frappe.db.exists("Energy Point Rule", id)

    if point_rule_exists:
        return frappe.get_doc("Energy Point Rule", id)
...
def assign_users_to_todo(todo_id, users):
    for user in users:
        assign_to({"assign_to": [user], "doctype": "ToDo", "id": todo_id})
