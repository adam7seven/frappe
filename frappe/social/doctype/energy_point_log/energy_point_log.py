...
def get_notification_message(doc):
    ...
    if doc.type == "Auto":
        ...
        message = message.format(
            owner_id, frappe.bold(points), doc.rule, get_title_html(title)
        )
        ...
        message = message.format(
            frappe.bold(owner_id), get_title_html(title), frappe.bold(points)
        )
...
def create_energy_points_log(ref_doctype, ref_id, doc, apply_only_once=False):
    ...
    if doc.rule:
        log_exists = check_if_log_exists(
            ref_doctype, ref_id, doc.rule, None if apply_only_once else doc.user
        )
        if log_exists:
...
def create_review_points_log(user, points, reason=None, doctype=None, docid=None):
    return frappe.get_doc(
        {
            "doctype": "Energy Point Log",
            "points": points,
