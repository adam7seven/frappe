...
def process_energy_points(doc, state):
    ...
    for d in frappe.cache_manager.get_doctype_map(
        ...
        frappe.get_doc("Energy Point Rule", d.get("id")).apply(doc)


def revert_points_for_cancelled_doc(doc):
    energy_point_logs = frappe.get_all(
        "Energy Point Log",
        {"reference_doctype": doc.doctype, "reference_id": doc.id, "type": "Auto"},
    )
    for log in energy_point_logs:
        reference_log = frappe.get_doc("Energy Point Log", log.id)
...
def get_energy_point_doctypes():
    return [
        ...
        for d in frappe.get_all(
            "Energy Point Rule", ["reference_doctype"], {"enabled": 1}
        )
    ]
