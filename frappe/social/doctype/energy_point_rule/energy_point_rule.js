...
frappe.ui.form.on("Energy Point Rule", {
    ...
    set_field_options(frm) {
        ...
        frappe.model.with_doctype(reference_doctype, () => {
            ...
            const fields = frappe.meta.get_docfields(frm.doc.reference_doctype).filter(frappe.model.is_value_type);

            const fields_to_check = fields.map(map_for_options);

            const user_fields = fields
                .filter(df => (df.fieldtype === "Link" && df.options === "User") || df.fieldtype === "Data")
                .map(map_for_options)
                .concat([
            ...
            const multiplier_fields = fields.filter(df => ["Int", "Float"].includes(df.fieldtype)).map(map_for_options);
