...
def upload_from_folder(
    ...
    for f in frappe.get_all(
        ...
        for file_metadata in response.entries:
            try:
                if os.path.basename(filepath) == file_metadata.id and os.stat(
                    encode(filepath)
                ).st_size == int(file_metadata.size):
                    found = True
                    update_file_dropbox_status(f.id)
                    break
            except Exception:
...
def is_fresh_upload():
    file_name = frappe.db.get_value("File", {"uploaded_to_dropbox": 1}, "id")
...
def get_dropbox_settings(redirect_uri=False):
    ...
    app_details = {
        ...
        "app_secret": (
            settings.get_password(fieldname="app_secret_key", raise_exception=False)
            if settings.app_secret_key
            else frappe.conf.dropbox_secret_key
        ),
        "refresh_token": settings.get_password(
            "dropbox_refresh_token", raise_exception=False
        ),
...
def delete_older_backups(dropbox_client, folder_path, to_keep):
    ...
    files = [
        ...
        if isinstance(f, dropbox.files.FileMetadata) and "sql" in f.id
    ]
    ...
    for f in files[to_keep:]:
        dropbox_client.files_delete(os.path.join(folder_path, f.id))
