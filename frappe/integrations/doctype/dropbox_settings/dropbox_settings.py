...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
    ...
    for f in frappe.get_all(
        ...
        for file_metadata in response.entries:
            try:
                if os.path.basename(filepath) == file_metadata.name and os.stat(encode(filepath)).st_size == int(
                    file_metadata.size
                ):
                    found = True
...
def delete_older_backups(dropbox_client, folder_path, to_keep):
    ...
    files = [f for f in res.entries if isinstance(f, dropbox.files.FileMetadata) and "sql" in f.id]
