...
def sync_contacts_from_google_contacts(g_contact):
    ...
    frappe.db.set_value(
        "Google Contacts", account.id, "last_sync_on", frappe.utils.now_datetime()
    )
                ...
                contact = frappe.get_doc(
                    {
                        ...
                        "company_id": get_indexed_value(
                            connection.get("organizations"), 0, "id"
                        ),
                    }
                )
...
def insert_contacts_to_google_contacts(doc, method=None):
    ...
    try:
        ...
        frappe.db.set_value(
            "Contact", doc.id, "google_contacts_id", contact.get("resourceName")
        )
    except HttpError as err:
        frappe.msgprint(
            _(
