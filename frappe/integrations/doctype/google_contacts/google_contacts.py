...
def sync_contacts_from_google_contacts(g_contact):
    ...
    while True:
        try:
            ...
            frappe.throw(
                _("Google Contacts - Could not sync contacts from Google Contacts {0}, error code {1}.").format(
        ...
        if not contacts.get("nextPageToken"):
            if contacts.get("nextSyncToken"):
                frappe.db.set_value("Google Contacts", account.id, "next_sync_token", contacts.get("nextSyncToken"))
                frappe.db.commit()
            break
...
def insert_contacts_to_google_contacts(doc, method=None):
    ...
    try:
        contact = (
            google_contacts.people()
            .createContact(body={"ids": [ids], "phoneNumbers": phoneNumbers, "emailAddresses": emailAddresses})
            .execute()
        )
