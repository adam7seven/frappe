...
class LDAPSettings(Document):
        ...
        if not self.flags.ignore_mandatory:
            if (
                ...
                if self.ldap_directory_server.lower() == "custom":
                    if (
                        not self.ldap_group_member_attribute
                        or not self.ldap_group_objectclass
                    ):
                        frappe.throw(
                            _(
                ...
                if (
                    self.ldap_custom_group_search
                    and "{0}" not in self.ldap_custom_group_search
                ):
                    frappe.throw(
                        _(
    ...
    def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
        try:
            ...
            bind_type = (
                AUTO_BIND_TLS_BEFORE_BIND if self.ssl_tls_mode == "StartTLS" else True
            )
    ...
    def fetch_ldap_groups(self, user: Entry, conn: ldap3.Connection) -> list:
        if not isinstance(user, Entry):
            raise TypeError(
                "Invalid type, attribute 'user' must be of type 'ldap3.abstract.entry.Entry'"
            )

        if not isinstance(conn, ldap3.Connection):
            raise TypeError(
                "Invalid type, attribute 'conn' must be of type 'ldap3.Connection'"
            )
