...
class LDAP_TestCase:
    ...
    def test_mandatory_fields(self):
        ...
        for mandatory_field in mandatory_fields:
            ...
            with contextlib.suppress(MandatoryError, ValidationError):
                frappe.get_doc(localdoc).save()
                self.fail(
                    f"Document LDAP Settings field [{mandatory_field}] is not mandatory"
                )
        ...
        for (
            ...
            if (
                non_mandatory_field == "doctype"
                or non_mandatory_field in mandatory_fields
            ):
                continue
            ...
            try:
                ...
                self.fail(
                    f"Document LDAP Settings field [{non_mandatory_field}] should not be mandatory"
                )
    ...
    def test_validation_ldap_search_string(self):
        ...
        for invalid_search_string in invalid_ldap_search_strings:
            ...
            with contextlib.suppress(ValidationError):
                frappe.get_doc(localdoc).save()
                self.fail(
                    f"LDAP search string [{invalid_search_string}] should not validate"
                )
    ...
    def test_connect_to_ldap(self):
        ...
        with mock.patch("ldap3.Server") as ldap3_server_method:
            with mock.patch(
                "ldap3.Connection", return_value=self.connection
            ) as ldap3_connection_method:
                with mock.patch("ldap3.Tls") as ldap3_Tls_method:
                    ...
                    for connection_arg in kwargs:
                        if (
                            ...
                            and kwargs[connection_arg]
                            in prevent_connection_parameters[connection_arg]
                        ):
                            self.fail(
                    ...
                    if local_doc["require_trusted_certificate"] == "Yes":
                        ...
                        tls_configuration = ldap3.Tls(
                            validate=tls_validate, version=tls_version
                        )
                        ...
                        self.assertTrue(
                            kwargs["auto_bind"], "ldap3.Connection must autobind"
                        )

                    ldap3_Tls_method.assert_called_with(
                        validate=tls_validate, version=tls_version
                    )
    ...
    def test_get_ldap_client_settings(self):
        ...
        self.assertTrue(
            result["enabled"] == self.doc["enabled"]
        )  # settings should match doc
    ...
    def test_sync_roles(self):
        ...
        role_to_group_map = {
            self.doc["ldap_groups"][0]["erpnext_role"]: self.doc["ldap_groups"][0][
                "ldap_group"
            ],
            self.doc["ldap_groups"][1]["erpnext_role"]: self.doc["ldap_groups"][1][
                "ldap_group"
            ],
            self.doc["ldap_groups"][2]["erpnext_role"]: self.doc["ldap_groups"][2][
                "ldap_group"
            ],
    ...
    def test_reset_password(self):
        ...
        with mock.patch(
            ...
            connect_to_ldap.assert_called_with(
                self.base_dn, self.base_password, read_only=False
            )
