        ...
        try:
            ...
            if err.resp.status == 410:
                set_encrypted_password(
                    "Google Calendar", account.id, "", "next_sync_token"
                )
                frappe.db.commit()
                msg += " " + _(
...
def update_event_in_google_calendar(doc, method=None):
    ...
    try:
        ...
        if doc.add_video_conferencing:
            event.update({"conferenceData": get_conference_data(doc)})
            conference_data_version = 1
        elif doc.get_doc_before_save().add_video_conferencing or event.get(
            "hangoutLink"
        ):
    ...
    for participant in doc.event_participants:
        if participant.get("email"):
            ...
            email_not_found.append(
                {"dt": participant.reference_doctype, "dn": participant.reference_docid}
            )
