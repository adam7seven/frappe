...
class GoogleCalendar(Document):
    ...
    def get_access_token(self):
        ...
        try:
            ...
            button_label = frappe.bold(_("Allow Google Calendar Access"))
            frappe.throw(
                _("Something went wrong during the token generation. Click on {0} to generate a new one.").format(
                    button_label
                )
            )
...
def get_google_calendar_object(g_calendar):
    ...
    google_calendar = build(serviceName="calendar", version="v3", credentials=credentials, static_discovery=False)
...
def google_calendar_to_repeat_on(start, end, recurrence=None):
    ...
    repeat_on = {
        "starts_on": (
            ...
            if start.get("date")
            else parser.parse(start.get("dateTime")).astimezone(ZoneInfo(get_system_timezone())).replace(tzinfo=None)
        ),
        "ends_on": (
            get_datetime(end.get("date"))
            if end.get("date")
            else parser.parse(end.get("dateTime")).astimezone(ZoneInfo(get_system_timezone())).replace(tzinfo=None)
        ),
