...
def get_context(context):
    ...
    if frappe.form_dict.q:
        ...
        context.update(
            get_search_results(
                query, frappe.utils.sanitize_html(frappe.form_dict.scope)
            )
        )
    else:
...
def get_search_results(
    text: str, scope: str | None = None, start: int = 0, as_html: bool = False
):
    ...
    if as_html:
        out.results = frappe.render_template(
            "templates/includes/search_result.html", out
        )
