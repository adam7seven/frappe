...
def get_context(context) -> PrintContext:
    ...
    if not (
        (frappe.form_dict.doctype and frappe.form_dict.name) or frappe.form_dict.doc
    ):
        return PrintContext(
            print_style="",
            comment="",
...
def get_print_format_doc(
    print_format_name: str, meta: "Meta"
...
def set_title_values_for_table_and_multiselect_fields(
...
def validate_print_permission(doc: "Document") -> None:
    ...
    if (
        ...
        and validate_key(key, doc) is not False
    ):
        return
...
def get_print_format(doctype: str, print_format: "PrintFormat") -> str:
    if print_format.disabled:
        frappe.throw(
            _("Print Format {0} is disabled").format(print_format.name),
...
def get_visible_columns(
    data: list, table_meta: "Meta", df: "DocField"
) -> list["DocField"]:
