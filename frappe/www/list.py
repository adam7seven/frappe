...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
    ...
    result = []
    row_template = (
        list_context.row_template or "templates/includes/list/row_template.html"
    )
    ...
    for doc in raw_result:
        ...
        new_context = frappe._dict(
            doc=doc, meta=meta, list_view_fields=list_view_fields
        )
...
def get_list_data(
    ...
    **kwargs,
    ...
    list_context = get_list_context(frappe._dict(), doctype, web_form_id)
...
def get_list_context(context, doctype, web_form_id=None):
    ...
    if meta.custom and web_form_id:
        webform_list_contexts = frappe.get_hooks("webform_list_context")
        if (
            webform_list_contexts
            and not frappe.get_doc("Module Def", meta.module).custom
        ):
            out = frappe._dict(
                frappe.get_attr(webform_list_contexts[0])(meta.module) or {}
            )
            if out:
                list_context = out
    ...
    if web_form_id:
        ...
        list_context = update_context_from_module(
            get_web_form_module(web_form), list_context
        )
    ...
    if txt:
        if meta.search_fields:
            or_filters.extend(
                ...
                in ("Data", "Text", "Small Text", "Text Editor")
            )
        else:
