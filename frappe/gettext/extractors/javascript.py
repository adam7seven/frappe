    ...
    for token in tokenize(
        ...
        if not funcname and token.type == "template_string":
            yield from parse_template_string(
                token.value, keywords, options, token.lineno
            )
        ...
        if token.type == "operator" and token.value == "(":
            ...
            and token.value in all_container_operators
        ):
            ...
            call_stack == -1 and token.type == "linecomment"
        ) or token.type == "multilinecomment":
            ...
            if character == "{" and prev_character == "$":
                ...
                if level == 0 and expression_contents:
                    ...
                    yield from extract_javascript(
                        expression_contents, keywords, options, lineno
                    )
