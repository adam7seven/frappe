context("Control Link", () => {
    ...
    it("should unset invalid value", () => {
        ...
        cy.get(".frappe-control[data-fieldname=link] input").type("invalid value", { delay: 100 }).blur();
    ...
    it("should update dependant fields (via fetch_from)", () => {
        cy.get("@todos").then(todos => {
            ...
            cy.wait("@validate_link");
            cy.get(".frappe-control[data-fieldname=assigned_by_full_name] .control-value").should("contain", "Frappe");

            cy.window().its("cur_frm.doc.assigned_by").should("eq", cy.config("testUser"));

            // invalid input
            cy.get("@input").clear().type("invalid input", { delay: 100 }).blur();
            cy.get(".frappe-control[data-fieldname=assigned_by_full_name] .control-value").should("contain", "");
